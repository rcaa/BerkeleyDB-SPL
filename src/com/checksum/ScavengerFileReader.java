 ...
 *
 * $Id: ScavengerFileReader.java,v 1.1.2.1 2006/10/12 16:08:24 ckaestne Exp $
 */
...
abstract public class ScavengerFileReader extends FileReader {
	 ...
	 */
	public ScavengerFileReader(EnvironmentImpl env, int readBufferSize,
			long startLsn, long finishLsn, long endOfFileLsn)
			throws IOException, DatabaseException {

		super(env, readBufferSize, false, startLsn, null, // single file
															// number
	...
	protected boolean processEntry(ByteBuffer entryBuffer)
		...
		entry.readEntry(entryBuffer, currentEntrySize, currentEntryTypeVersion,
				true);
	...
	/*
	 * Method overriden by the caller. Each entry of the types selected is
	 * passed to this method.
	 */
	abstract protected void processEntryCallback(LogEntry entry,
			LogEntryType entryType) throws DatabaseException;

	/*
	 * Read the next entry. If a checksum exception is encountered, attempt to
	 * find the other side of the corrupted area and try to re-read this file.
	 */
	public boolean readNextEntry() throws DatabaseException, IOException {
		...
		try {
			...
			resyncReader(
					DbLsn.makeLsn(readBufferFileNum, saveCurrentEntryOffset),
					dumpCorruptedBounds);
			return super.readNextEntry();
		}
	}
	...
	protected boolean resyncReader(long nextGoodRecordPostCorruption,
			boolean showCorruptedBounds) throws DatabaseException, IOException {

		LastFileReader reader = null;
		long tryReadBufferFileNum = DbLsn
				.getFileNumber(nextGoodRecordPostCorruption);

		while (tryReadBufferFileNum >= 0) {
			try {
				reader = new LastFileReader(env, readBufferSize, new Long(
						tryReadBufferFileNum));
				break;
			} catch (DbChecksumException DCE) {

				/*
				 * We found a checksum error reading the header of this file so
				 * skip to a completely earlier file.
				 */
				tryReadBufferFileNum--;
				continue;
			}
		...
		boolean switchedFiles = tryReadBufferFileNum != DbLsn
				.getFileNumber(nextGoodRecordPostCorruption);
		...
		if (showCorruptedBounds) {
			...
			System.err
					.println("Next known good record after corruption is at LSN:\n   "
