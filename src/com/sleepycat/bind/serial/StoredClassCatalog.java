 ...
 *
 * $Id: StoredClassCatalog.java,v 1.1.6.3.2.2 2006/10/23 21:07:31 ckaestne Exp $
 */
...
public class StoredClassCatalog implements ClassCatalog {
	 ...
	 * 
	 * [0] [next class ID] [1 / class ID] [ObjectStreamClass (class format)] [2 /
	 * class name] [ClassInfo (has 8 byte class ID)]
	 */
	 ...
	 * 
	 * @param database
	 *            an open database to use as the class catalog. It must be a
	 ...
	 *             if the database is not a BTREE database or if it configured
	 *             to allow duplicates.
	 */
	public StoredClassCatalog(Database database) throws DatabaseException,
		...
		if (dbConfig.getReadOnly()) {
			...
			OperationStatus status = db.get((OperationContext)null, key, data, null);
			...
			db.putNoOverwrite((OperationContext)null, key, data);
		}
	}
	...
	private ObjectStreamClass getClassFormat(byte[] classID, DatabaseEntry data)
		...
		ObjectStreamClass classFormat = (ObjectStreamClass) formatMap
				.get(classIDObj);
		if (classFormat == null) {
			...
			OperationStatus status = db.get((OperationContext)null, key, data,
					LockMode.DEFAULT);
			if (status != OperationStatus.SUCCESS) {
			...
			try {
				ObjectInputStream ois = new ObjectInputStream(
						new ByteArrayInputStream(data.getData(), data
	...
	private ClassInfo getClassInfo(ObjectStreamClass classFormat)
		...
		/*
		 * First check for a cached copy of the class info, which if present
		 * always contains the class format object
		 */
		...
		if (classInfo != null) {
			...
			if (status != OperationStatus.SUCCESS) {
				...
				ObjectStreamClass storedClassFormat = getClassFormat(classInfo
						.getClassID(), formatData);
				...
				if (!areClassFormatsEqual(storedClassFormat,
						getBytes(formatData), classFormat)) {
	...
	private ClassInfo putClassInfo(ClassInfo classInfo, String className,
			DatabaseEntry classKey, ObjectStreamClass classFormat)
			throws DatabaseException, ClassNotFoundException {
		...
		Cursor cursor = null;
		try {
			cursor = db.openCursor((OperationContext)null, cursorConfig);
			...
			/*
			 * Write the new class info record, using the key passed in; this is
			 * done last so that a reader who gets the class info record first
			 * will always find the corresponding class format record.
			 */
	...
	private static boolean areClassFormatsEqual(ObjectStreamClass format1,
			byte[] format1Bytes, ObjectStreamClass format2) {
		try {
