 ...
 *
 * $Id: SerialBinding.java,v 1.1.6.1 2006/07/28 09:02:38 ckaestne Exp $
 */
...
public class SerialBinding extends SerialBase implements EntryBinding {
	 ...
	 * 
	 * @param baseClass
	 *            is the base class for serialized objects stored using this
	...
	public Object entryToObject(DatabaseEntry entry) {
		...
		System.arraycopy(entry.getData(), entry.getOffset(), bufWithHeader,
				hdr.length, length);

		try {
			SerialInput jin = new SerialInput(new FastInputStream(
					bufWithHeader, 0, bufWithHeader.length), classCatalog,
					getClassLoader());
	 ...
	 * 
	 * @param object
	 *            is the input deserialized object.
	 * 
	 * @param entry
	 *            is the output serialized entry.
	 * 
	 * @throws IllegalArgumentException
	 *             if the object is not an instance of the base class for this
	 *             binding.
	 */
	public void objectToEntry(Object object, DatabaseEntry entry) {

		if (baseClass != null && !baseClass.isInstance(object)) {
			throw new IllegalArgumentException("Data object class ("
					+ object.getClass()
		...
		byte[] hdr = SerialOutput.getStreamHeader();
		entry.setData(fo.getBufferBytes(), hdr.length, fo.getBufferLength()
				- hdr.length);
	}
}
