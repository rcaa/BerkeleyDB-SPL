 ...
 *
 * $Id: TupleBase.java,v 1.1.6.1 2006/07/28 09:02:30 ckaestne Exp $
 */
...
public class TupleBase {
	 ...
	 * 
	 * <p>
	 * Unless {@link #setTupleBufferSize} is called, the default {@link
	...
	public static void outputToEntry(TupleOutput output, DatabaseEntry entry) {
		...
		entry.setData(output.getBufferBytes(), output.getBufferOffset(), output
				.getBufferLength());
	}
	...
	public static void inputToEntry(TupleInput input, DatabaseEntry entry) {
		...
		entry.setData(input.getBufferBytes(), input.getBufferOffset(), input
				.getBufferLength());
	}
	...
	public static TupleInput entryToInput(DatabaseEntry entry) {
		...
		return new TupleInput(entry.getData(), entry.getOffset(), entry
				.getSize());
	}
