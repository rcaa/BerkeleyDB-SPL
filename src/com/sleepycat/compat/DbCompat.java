 ...
 *
 * $Id: DbCompat.java,v 1.1.6.3.2.1 2006/10/23 21:07:28 ckaestne Exp $
 */
...
/**
 * A minimal set of DB-JE compatibility methods for internal use only. Two
 * versions are maintained in parallel in the DB and JE source trees. Used by
...
public class DbCompat {
	...
	public static OperationStatus getCurrentRecordNumber(Cursor cursor,
			DatabaseEntry key, LockMode lockMode) throws DatabaseException {
		throw new UnsupportedOperationException();
	}
	...
	public static OperationStatus putAfter(Cursor cursor, DatabaseEntry key,
			DatabaseEntry data) throws DatabaseException {
		throw new UnsupportedOperationException();
	}

	public static OperationStatus putBefore(Cursor cursor, DatabaseEntry key,
			DatabaseEntry data) throws DatabaseException {
		throw new UnsupportedOperationException();
	}

	public static OperationStatus append(Database db, /*ChK_ Transaction txn,*/
	...
	public static Database openDatabase(Environment env, OperationContext cxt,
			String file, String name, DatabaseConfig config)
			throws DatabaseException, FileNotFoundException {
		return env.openDatabase(cxt, makeDbName(file, name), config);
	}

	public static SecondaryDatabase openSecondaryDatabase(Environment env,
			OperationContext cxt, String file, String name, Database primary,
			SecondaryConfig config) throws DatabaseException,
			FileNotFoundException {
		return env.openSecondaryDatabase(cxt, makeDbName(file, name), primary,
				config);
	}
