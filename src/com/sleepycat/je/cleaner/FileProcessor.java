 ...
 *
 * $Id: FileProcessor.java,v 1.3.6.2.2.11 2006/10/25 19:31:57 ckaestne Exp $
 */
...
public class FileProcessor {
	...
	public synchronized int doClean(boolean invokedFromDaemon,
		...
		while (true) {
			 ...
			 */
			if (hook_cleanFile(fileNum, invokedFromDaemon))
				nFilesCleaned += 1;
	...
	private boolean hook_cleanFile(Long fileNum, boolean invokedFromDaemon)
			throws DatabaseException {
		...
		try {
			...
			String traceMsg = hook_getBeginTraceMenssage(runId, fileNumValue);
			// refined trace Tracer.trace(Level.INFO, env, traceMsg);
			if (DEBUG_TRACING) {
				System.out.println("\n" + traceMsg);
			}
			...
			// refined trace Tracer.trace(env, "Cleaner", "doClean", "",
			// IOE);
			...
			String traceMsg = getEndTraceMenssage(runId, fileNumValue,
					invokedFromDaemon, finished);
			// refined trace Tracer.trace(Level.SEVERE, env, traceMsg);
			if (DEBUG_TRACING) {
				System.out.println("\n" + traceMsg);
			}
		}
	...
	private String getEndTraceMenssage(int runId, long fileNumValue,
			boolean invokedFromDaemon, boolean finished) {
		return "CleanerRun " + runId + " on file 0x"
				+ Long.toHexString(fileNumValue) + " invokedFromDaemon="
				+ invokedFromDaemon + " finished=" + finished;

	}

	private String hook_getBeginTraceMenssage(int runId, long fileNumValue) {
		return "CleanerRun " + runId + " on file 0x"
				+ Long.toHexString(fileNumValue) + " begins";
	...
	private boolean processFile(Long fileNum) throws DatabaseException,
		...
		return hookr_processFileInternal(fileNum, obsoleteOffsets, tfs,
				obsoleteIter, nextObsolete, readBufferSize);

	}

	private boolean hookr_processFileInternal(Long fileNum,
			PackedOffsets obsoleteOffsets, TrackedFileSummary tfs,
			PackedOffsets.Iterator obsoleteIter, long nextObsolete,
		...
		try {
			...
			if (!hookr_processFileInternalLoop(fileNum, tfs, obsoleteIter,
					nextObsolete, checkPendingDbSet, dbCache, reader,
					dbMapTree, location))
				return false;
			...
			for (Iterator i = checkPendingDbSet.iterator(); i.hasNext();) {
				...
				DatabaseImpl db = dbMapTree.getDbCaching(dbId,
	...
	private boolean hookr_processFileInternalLoop(Long fileNum,
			TrackedFileSummary tfs, PackedOffsets.Iterator obsoleteIter,
			long nextObsolete, Set checkPendingDbSet, Map dbCache,
			CleanerFileReader reader, DbTree dbMapTree, TreeLocation location)
			throws DatabaseException, IOException {
		int nProcessedLNs = 0;
		while (reader.readNextEntry()) {
			...
			if (isLN) {
				...
				processLN(fileNum, location,
						new Long(DbLsn.getFileOffset(lsn)), new LNInfo(
								targetLN, dbId, key, dupKey), dbCache);

				/*
				 * Process pending LNs before proceeding in order to prevent the
				 * pending list from growing too large.
				 */
				nProcessedLNs += 1;
	...
	private void hook_obsoleteRun(boolean isLN, boolean isIN) {
		// TODO Auto-generated method stub

	}

	private void hook_finishedReading(CleanerFileReader reader) {
	...
	private void processLN(Long fileNum, TreeLocation location, Long offset,
		...
		DatabaseImpl db = env.getDbMapTree().getDbCaching(info.getDbId(),
				cleaner.lockTimeout, dbCache);
		...
		try {
			 ...
			 */
			if (cleaner.hook_checkDeletedDb(db)) {
				obsolete = true;
				completed = true;
				return;
			}
			...
			if (isDupCountLN) {
				...
				// Lck parentDIN.latch(Cleaner.UPDATE_GENERATION);
			...
			if (processedHere) {
				// refined trace cleaner.trace(cleaner.detailedTraceLevel,
				// Cleaner.CLEAN_LN, ln, logLsn, completed, obsolete, false
				// /*migrated*/);
			}
		}
	}

	private void hook_entryIsKnownDeleted() {
		// TODO Auto-generated method stub
	...
	private void processFoundLN(LNInfo info, long logLsn, long treeLsn,
		...
		try {
			...
			// refined trace cleaner.trace(cleaner.detailedTraceLevel,
			// Cleaner.CLEAN_LN, ln, logLsn, completed, obsolete, migrated);
		}
	}

	private void hook_lnsMarked() {
		// TODO Auto-generated method stub
	...
	private void processIN(IN inClone, DatabaseImpl db, long lsn)
		...
		try {
			...
			// refined trace cleaner.trace(cleaner.detailedTraceLevel,
			// Cleaner.CLEAN_IN, inClone, lsn, completed, obsolete, dirtied);
		}
	}
	...
	private IN findINInTree(Tree tree, DatabaseImpl db, IN inClone, long lsn)
		...
		// Lck inClone.latch(Cleaner.UPDATE_GENERATION);
		...
		int compareVal = DbLsn.compareTo(result.parent.getLsn(result.index),
				lsn);
		...
		if (compareVal > 0) {
			...
			/*
			 * Log entry is same or newer than what's in the tree. Dirty the IN
			 * and let checkpoint write it out.
			 */
			IN in;
			if (compareVal == 0) {
			...
			// Lck in.latch(Cleaner.UPDATE_GENERATION);
			return in;
		}
		// } finally {
		// Lck if ((result != null) && (result.exactParentFound)) {
	...
	private static class RootDoWork implements WithRootLatched {
		...
		public IN doWork(ChildReference root) throws DatabaseException {
			...
			if (DbLsn.compareTo(root.getLsn(), lsn) <= 0) {
				...
				// Lck rootIN.latch(Cleaner.UPDATE_GENERATION);
				return rootIN;
			} else {
				return null;
			}
