 ...
 *
 * $Id: FileSummary.java,v 1.1.6.1.2.2 2006/10/23 17:22:02 ckaestne Exp $
 */
...
public class FileSummary implements LogWritable, LogReadable {
	...
	public boolean isEmpty() {
		...
		return totalCount == 0 && totalSize == 0 && obsoleteINCount == 0
				&& obsoleteLNCount == 0;
	}
	...
	public int getObsoleteSize() throws DatabaseException {
		...
		if (totalSize > 0) {
			...
			int obsoleteSize = getObsoleteLNSize() + getObsoleteINSize()
					+ leftoverSize;

			/*
			 * Don't report more obsolete bytes than the total. We may calculate
			 * more than the total because of (intentional) double-counting
			 * during recovery.
			 */
	...
	/**
	 * Returns the total number of entries counted. This value is guaranted to
	 * increase whenever the tracking information about a file changes. It is
	 * used a key discriminator for FileSummaryLN records.
	 */
	...
	public void readFromLog(ByteBuffer buf, byte entryTypeVersion) {
		...
		if (obsoleteINCount == -1) {
			 ...
			 * earlier, consider all INs to be obsolete. This causes the file to
			 * be cleaned, and then IN counting will be accurate.
			 */
