 ...
 *
 * $Id: UtilizationProfile.java,v 1.1.6.2.2.8 2006/10/26 16:36:48 ckaestne Exp $
 */
...
public class UtilizationProfile implements EnvConfigObserver {
	...
	/*
	 * Note that age is a distance between files not a number of files, that is,
	 * deleted files are counted in the age.
	 */
	...
	private FileSummaryMap fileSummaryMap; // cached fileNum -> FileSummary
	static class FileSummaryMap extends TreeMap{}
	...
	/**
	 * An array of pairs of file numbers, where each pair is a range of files to
	 * be force cleaned. Index i is the from value and i+1 is the to value, both
	 * inclusive.
	 */
	...
	public UtilizationProfile(EnvironmentImpl env, UtilizationTracker tracker)
		...
		fileSummaryMap = new FileSummaryMap();
	 ...
	 * 
	 * @param fileSelector
	 *            is used to determine valid cleaning candidates.
	 * 
	 * @param forceCleaning
	 *            is true to always select a file, even if its utilization is
	 *            above the minimum utilization threshold.
	 * 
	 * @param lowUtilizationFiles
	 *            is a returned set of files that are below the minimum
	 *            utilization threshold.
	 */
	synchronized Long getBestFileForCleaning(FileSelector fileSelector,
			boolean forceCleaning, Set lowUtilizationFiles)
			throws DatabaseException {
		...
		while (iter.hasNext()) {
			...
			/*
			 * If the file is already being cleaned, only total the non-obsolete
			 * amount. This is an optimistic prediction of the results of
			 * cleaning, and is used to prevent over-cleaning. Update the total
			 * obsolete size to include the utilization DB records that will be
			 * deleted when the log file is deleted.
			 */
		...
		/*
		 * Return the best file if we are under the minimum utilization or we're
		 * cleaning aggressively.
		 */
		int totalUtilization = utilization(totalObsoleteSize, totalSize);
		if (forceCleaning || totalUtilization < useMinUtilization
	...
	private int estimateUPObsoleteSize(FileSummary summary) {
		...
		/*
		 * FileSummaryLN overhead: 14 Header 8 Node 1 Deleted 4 Data Length (0)
		 * 32 Base Summary (8 X 4) 8 PackedOffsets size and length (2 * 4) 8
		 * PackedOffsets first offset
		 */
		final int OVERHEAD = 75;

		/*
		 * Make an arbitrary estimate of the number of offsets per
		 * FileSummaryLN. Then estimate the total byte size, assuming all short
		 * (2 byte) offsets. Round up the total number of log entries.
		 */
			...
			while (tokens.hasMoreTokens()) {
				...
				try {
					...
					throw new IllegalArgumentException(errPrefix
							+ "Invalid hex file number: " + fromStr);
				}
					...
					if (",".equals(delim)) {
						...
						if (tokens.hasMoreTokens()) {
							...
							try {
								...
								throw new IllegalArgumentException(errPrefix
										+ "Invalid hex file number: " + toStr);
							}
						} else {
							throw new IllegalArgumentException(errPrefix
									+ "Expected file number: " + delim);
						}
					} else {
						throw new IllegalArgumentException(errPrefix
								+ "Expected '-' or ',': " + delim);
	...
	public void countAndLogSummaries(TrackedFileSummary[] summaries)
		...
		if (!DbInternal.getCheckpointUP(env.getConfigManager()
	...
	private void deleteFileSummary(Long fileNum) throws DatabaseException {
		...
		try {
			...
			while (status == OperationStatus.SUCCESS) {
				...
				FileSummaryLN ln = (FileSummaryLN) cursor
				...
				status = cursor.getNext(keyEntry, dataEntry, LockType.WRITE,
						true // forward
						/*Lck false*/); // alreadyLatched
			}
		} finally {
			if (cursor != null) {
//				Lck cursor.releaseBINs();
				cursor.close();
			}
			if (locker != null) {
				locker.operationEnd();
	...
	private synchronized PackedOffsets putFileSummary(TrackedFileSummary tfs)
		...
		if (summary == null) {
			 ...
			 * profile record if no corresponding log file exists.
			 */
			File file = new File(env.getFileManager().getFullFileName(fileNum,
	 ...
	 * used to test for obsolete offsets that are being added during cleaning by
	 * other threads participating in lazy migration. The caller must call
	 * TrackedFileSummary.setAllowFlush(true) when cleaning is complete. This
	 * method performs eviction and is not synchronized.
	 * 
	 * @param logUpdate
	 *            if true, log any updates to the utilization profile. If false,
	 *            only retrieve the new information.
	 */
	TrackedFileSummary getObsoleteDetail(Long fileNum,
			PackedOffsets packedOffsets, boolean logUpdate)
			throws DatabaseException {
		...
		/*
		 * Get an unflushable summary that will remain valid for the duration of
		 * file cleaning.
		 */
		TrackedFileSummary tfs = env.getLogManager()
				.getUnflushableTrackedSummary(fileNumVal);
		...
		try {
			...
			if (cursor.positionFirstOrLast(true, null)) {
				...
				OperationStatus status = cursor.getCurrent(
						keyEntry, dataEntry, LockType.NONE, true);
...
//							Lck cursor.releaseBIN();
					...
					if (isOldVersion) {
						 ...
						 */
						if (!getFirstFSLN(cursor, fileNum + 1, keyEntry,
								dataEntry, LockType.NONE)) {
							status = OperationStatus.NOTFOUND;
						}
					}
				}
...
//				Lck cursor.releaseBINs();
	 ...
	 */
	private boolean getFirstFSLN(CursorImpl cursor, long fileNum,
			DatabaseEntry keyEntry, DatabaseEntry dataEntry, LockType lockType)
			throws DatabaseException {

		byte[] keyBytes = FileSummaryLN.makePartialKey(fileNum);
		keyEntry.setData(keyBytes);

		int result = cursor.searchAndPosition(keyEntry, dataEntry,
				SearchMode.SET_RANGE, lockType);
		...
		if (exactKeyMatch
				&& cursor.getCurrent(keyEntry, dataEntry,
						lockType, true) != OperationStatus.KEYEMPTY) {
			return true;
		}

		OperationStatus status = cursor.getNext(keyEntry, dataEntry, lockType,
				true // forward
				/*Lck, !exactKeyMatch*/); // alreadyLatched
	...
	private boolean openFileSummaryDatabase() throws DatabaseException {
		...
		try {
			...
			DatabaseImpl db = dbTree.getDb(autoTxn, DbTree.UTILIZATION_DB_NAME,
					null /*,true /* Eviction allowed. */);
			if (db == null) {
				if (env.isReadOnly()) {
					return false;
				}
				db = dbTree
						.createDb(autoTxn, DbTree.UTILIZATION_DB_NAME,
								new DatabaseConfig(), null /*,true /*
																	 * Eviction
																	 * allowed
																	 */);
			}
	 ...
	 */
	private synchronized void insertFileSummary(FileSummaryLN ln, long fileNum,
			int sequence) throws DatabaseException {
		...
		try {
			...
			if (status == OperationStatus.KEYEXIST) {
				hook_traceCleanerDup(fileNum, sequence);
			}
	...
	private void hook_traceCleanerDup(long fileNum, int sequence) {
	...
	public boolean verifyFileSummaryDatabase() throws DatabaseException {
		...
		try {
			...
			if (cursor.positionFirstOrLast(true, null)) {
				...
				OperationStatus status = cursor.getCurrent(key,
						data, LockType.NONE, true);
				...
				while (status == OperationStatus.SUCCESS) {
					...
					if (ln != null) {
						...
						hook_evictCursor(cursor);
						status = cursor.getNext(key, data, LockType.NONE, true // forward
								/*Lck false*/); // already latched
					}
				}
			}
		} finally {
	...
	private void hook_evictCursor(CursorImpl cursor) throws DatabaseException {
	}
	...
	private boolean verifyLsnIsObsolete(long lsn) throws DatabaseException {
		...
		/*
		 * The whole database is gone, so this LN is obsolete. No need to worry
		 * about delete cleanup; this is just verification and no cleaning is
		 * done.
		 */
		if (hook_isDbGone(db)) {
			return true;
		...
		try {
			...
			boolean parentFound = tree.getParentBINForChildLN(location, entry
					.getKey(), entry.getDupKey(), entry.getLN(), false, // splitsAllowed
...
//Lck 				bin.releaseLatch();
	...
	private boolean hook_isDbGone(DatabaseImpl db) {
		return db == null;	
	}
}
