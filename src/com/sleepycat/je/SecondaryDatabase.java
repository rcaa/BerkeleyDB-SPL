 ...
 *
 * $Id: SecondaryDatabase.java,v 1.2.6.3.2.1 2006/10/23 21:07:25 ckaestne Exp $
 */
...
public class SecondaryDatabase extends Database {
	...
	SecondaryDatabase(Environment env, SecondaryConfig secConfig,
			Database primaryDatabase) throws DatabaseException {
		...
		if (env.getEnvironmentImpl() != primaryDatabase.getEnvironment()
				.getEnvironmentImpl()) {
			throw new IllegalArgumentException(
	 ...
	 */
	void initExisting(Environment env, Locker locker, DatabaseImpl database,
			DatabaseConfig dbConfig) throws DatabaseException {
	 ...
	 * doc_src directory.
	 */
	public SecondaryCursor openSecondaryCursor(OperationContext cxt, 
			CursorConfig cursorConfig) throws DatabaseException {

		return (SecondaryCursor) openCursor(cxt, cursorConfig);
	}
	...
	Cursor newDbcInstance(OperationContext cxt, CursorConfig cursorConfig)
		...
		return new SecondaryCursor(this,cxt,cursorConfig);
	}
	...
	public OperationStatus delete(OperationContext cxt, DatabaseEntry key)
		...
		// refined trace trace(Level.FINEST, "SecondaryDatabase.delete", txn,
		// key, null, null);
		...
		try {
			...
			OperationStatus searchStatus = cursor.search(key, pKey,
	...
	public OperationStatus get(OperationContext cxt,  DatabaseEntry key,
		...
		return get(cxt, key, new DatabaseEntry(), data, lockMode);
	}
	...
	public OperationStatus get(OperationContext cxt, DatabaseEntry key,
		...
		// refined trace trace(Level.FINEST, "SecondaryDatabase.get", txn, key,
		// null, lockMode);
		...
		try {
			cursor = new SecondaryCursor(this, cxt, cursorConfig);
	 ...
	 * doc_src directory.
	 */
	public OperationStatus getSearchBoth(OperationContext cxt, DatabaseEntry key,
			DatabaseEntry data, LockMode lockMode) throws DatabaseException {
	...
	public OperationStatus getSearchBoth(OperationContext cxt, DatabaseEntry key,
		...
		// refined trace trace(Level.FINEST, "SecondaryDatabase.getSearchBoth",
		// txn, key, data,lockMode);
	 ...
	 * doc_src directory.
	 */
	public OperationStatus putNoOverwrite(OperationContext cxt, DatabaseEntry key,
			DatabaseEntry data) throws DatabaseException {
	 ...
	 * doc_src directory.
	 */
	public OperationStatus putNoDupData(OperationContext cxt, DatabaseEntry key,
			DatabaseEntry data) throws DatabaseException {
	 ...
	 * @deprecated
	 */
	public int truncate(OperationContext cxt, boolean countRecords)
			throws DatabaseException {
	 ...
	 * 
	 * @param cursor
	 *            secondary cursor to use, or null if this method should open
	 *            and close a cursor if one is needed.
	 * 
	 * @param priKey
	 *            the primary key.
	 * 
	 ...
	 *            been deleted.
	 */
	void updateSecondary(Locker locker, Cursor cursor, DatabaseEntry priKey,
		 ...
		 */
		if (secondaryConfig.getImmutableSecondaryKey() && oldData != null
				&& newData != null) {
			return;
		}
		...
		if (keyCreator != null) {
			...
			SecondaryMultiKeyCreator multiKeyCreator = secondaryConfig
					.getMultiKeyCreator();
			...
			if (oldData != null) {
				oldKeys = new HashSet();
				multiKeyCreator.createSecondaryKeys(this, priKey, oldData,
						oldKeys);
			}
			if (newData != null) {
				newKeys = new HashSet();
				multiKeyCreator.createSecondaryKeys(this, priKey, newData,
						newKeys);
			}
	 ...
	 */
	private void deleteKey(Cursor cursor, DatabaseEntry priKey,
			DatabaseEntry oldSecKey) throws DatabaseException {

		OperationStatus status = cursor.search(oldSecKey, priKey, LockMode.RMW,
				SearchMode.BOTH);
	 ...
	 */
	private void insertKey(Locker locker, Cursor cursor, DatabaseEntry priKey,
			DatabaseEntry newSecKey) throws DatabaseException {
		...
		if (foreignDb != null) {
			...
			try {
				...
				OperationStatus status = foreignCursor.search(newSecKey,
						tmpData, LockMode.DEFAULT, SearchMode.SET);
				if (status != OperationStatus.SUCCESS) {
					throw new DatabaseException("Secondary " + getDebugName()
		...
		if (status != OperationStatus.SUCCESS) {
			throw new DatabaseException("Could not insert secondary key in "
					+ getDebugName() + ' ' + status);
		}
	}
	...
	void onForeignKeyDelete(Locker locker, DatabaseEntry secKey)
		...
		ForeignKeyDeleteAction deleteAction = secondaryConfig
				.getForeignKeyDeleteAction();

		/* Use RMW if we're going to be deleting the secondary records. */
		LockMode lockMode = (deleteAction == ForeignKeyDeleteAction.ABORT) ? LockMode.DEFAULT
				: LockMode.RMW;
		...
		try {
			...
			while (status == OperationStatus.SUCCESS) {
				...
				if (deleteAction == ForeignKeyDeleteAction.ABORT) {
					...
					/*
					 * ABORT - throw an exception to cause the user to abort the
					 * transaction.
					 */
					throw new DatabaseException("Secondary " + getDebugName()
					...
					/*
					 * NULLIFY - set the secondary key to null in the associated
					 * primary record.
					 */
					Cursor priCursor = null;
					try {
						...
						status = priCursor.search(priKey, data, LockMode.RMW,
								SearchMode.SET);
						if (status == OperationStatus.SUCCESS) {
							ForeignMultiKeyNullifier multiNullifier = secondaryConfig
									.getForeignMultiKeyNullifier();
							if (multiNullifier != null) {
								if (multiNullifier.nullifyForeignKey(this,
										priKey, data, secKey)) {
									priCursor.putCurrent(data);
								}
							} else {
								ForeignKeyNullifier nullifier = secondaryConfig
	 ...
	 * Logger logger = envHandle.getEnvironmentImpl().getLogger(); if
	 * (logger.isLoggable(level)) { StringBuffer sb = new StringBuffer();
	 * sb.append(methodName); sb.append(" name=").append(getDebugName());
