 ...
 *
 * $Id: FileSummaryLN.java,v 1.1.6.1 2006/07/28 09:02:00 ckaestne Exp $
 */
 ...
 * 
 * <p>
 * The contents of the FileSummaryLN are not fixed until the moment at which the
 * LN is added to the log. A base summary object contains the summary last added
 * to the log. A tracked summary object contains live summary info being updated
 * in real time. The tracked summary is added to the base summary just before
 * logging it, and then the tracked summary is reset. This ensures that the
 * logged summary will accurately reflect the totals calculated at the point in
 * the log where the LN is added.
 * </p>
 * 
 * <p>
 * This is all done in the writeToLog method, which operates under the log write
 * latch. All utilization tracking must be done under the log write latch.
 * </p>
 * 
 * <p>
 * In record version 1, obsolete offset tracking was added and multiple records
 * are stored for a single file rather than a single record. Each record
 * contains the offsets that were tracked since the last record was written.
 * 
 * <p>
 * The key is 8 bytes: 4 bytes for the file number followed by 4 bytes for the
 * sequence number. The lowest valued key for a given file contains the most
 * recent summary information, while to get a complete list of obsolete offsets
 * all records for the file must be read. A range search using just the first 4
 * bytes can be used to find the most recent record -- this is possible because
 * the sequence number values are decreasing over time for a given file. Here
 * are example keys for three summary records in file 1:
 * </p>
 * 
 * <pre>
 ...
 * </p>
 * <p>
 * Version 1: Keys are two 4 byte integers: {file, sequence}. Obsolete detail is
 * present. Some offsets may be invalid if RMW was used.
 * </p>
 * <p>
 * Version 2: The RMW problem with invalid offsets was corrected. There is no
 * data format change; all versions of JE 2.0.x can read version 1.
 * </p>
 * 
...
public final class FileSummaryLN extends LN {
	 ...
	 * LN may be migrated by the cleaner, in which case the version will be 1 or
	 * greater [#13061]. In the latter case, we can distinguish a string key by:
	 * 
	 ...
	 * must be a string key. bytes[4] to bytes[7] are a sequence number that is
	 * the number of log entries counted. For this number to be greater than
	 * 0x30000000, the binary value of 4 digits starting with ascii "0", over
	 * 400 million log entries would have to occur in a single file; this should
	 * never happen.
	 * 
	 ...
	 * more than 400 million log entries per file, and more than 10 million were
	 * written with JE 1.7.1 or earlier.
	 */
	...
	/**
	 * Convert a FileSummaryLN key from a byte array to a long. The file number
	 * is the first 4 bytes of the key.
	 */
	...
	/**
	 * Returns the first 4 bytes of the key for the given file number. This can
	 * be used to do a range search to find the first LN for the file.
	 */
	 ...
	 * 
	 * @param sequence
	 *            is a unique identifier for the LN for the given file, and must
	 *            be greater than the last sequence.
	 */
	...
	public void postFetchInit(DatabaseImpl db, long sourceLsn)
		...
		if (logVersion == 1
				&& db.getDbEnvironment().getUtilizationProfile()
						.isRMWFixEnabled()) {
			obsoleteOffsets = new PackedOffsets();
		}
	}
	...
	/**
	 * @see LoggableObject#marshallOutsideWriteLatch FileSummaryLNs must be
	 *      marshalled within the log write latch, because that critical section
	 *      is used to guarantee that all previous log entries are reflected in
	 *      the summary.
	 */
	...
	public void writeToLog(ByteBuffer logBuffer) {
		 ...
		 * to the log, and reset the tracked summary. Do this even when deleting
		 * the LN, so that the tracked summary is cleared.
		 */
