 ...
 *
 * $Id: DIN.java,v 1.1.6.1.2.4 2006/10/25 17:21:15 ckaestne Exp $
 */
...
//	}
	 ...
	 * Create a new DIN.
	 */
	public DIN(DatabaseImpl db, byte[] identifierKey, int capacity,
			byte[] dupKey, ChildReference dupCountLNRef, int level) {
	...
	/**
	 * Create a new DIN. Need this because we can't call newInstance() without
	 * getting a 0 node.
	 */
	protected IN createNewInstance(byte[] identifierKey, int maxEntries,
			int level) {
		return new DIN(getDatabase(), identifierKey, maxEntries, dupKey,
				dupCountLNRef, level);
	}
	...
	/**
	 * Return the comparator function to be used for DINs. This is the user
	...
	/**
	 * Increment or decrement the DupCountLN, log the updated LN, and update the
	 * lock result.
	 * 
	 * Preconditions: This DIN is latched and the DupCountLN is write locked.
	 * Postconditions: Same as preconditions.
	 */
	public void incrementDuplicateCount(LockResult lockResult, byte[] key,
			Locker locker, boolean increment) throws DatabaseException {
		...
		long newCountLSN = dupCountLN.log(db.getDbEnvironment(), db.getId(),
				key, oldLsn, locker);
	...
	protected long logInternal(LogManager logManager, boolean allowDeltas,
		...
		if (dupCountLNRef != null) {
			...
			cleaner.lazyMigrateDupCountLN(this, dupCountLNRef,
					proactiveMigration);
		}

		return super.logInternal(logManager, allowDeltas, isProvisional,
