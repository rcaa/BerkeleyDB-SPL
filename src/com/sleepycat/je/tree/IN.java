 ...
 *
 * $Id: IN.java,v 1.4.6.2.2.8 2006/10/26 16:36:46 ckaestne Exp $
 */
...
public class IN extends Node implements Comparable, LoggableObject, LogReadable {
	...
	/*
	 * Levels: The mapping tree has levels in the 0x20000 -> 0x2ffffnumber
	...
	// FEATURE inlining suitable as new feature?
	/*
	 ...
	 * overhead of ChildReference and DbLsn objects by in-lining the elements of
	 * the ChildReference directly in the IN.
	 */
	 ...
	 * 
	 * If either (1) the difference in file numbers exceeds 127 (Byte.MAX_VALUE)
	 * or (2) the file offset is greater than 16,777,214, then the byte[] based
	 * rep mutates to a long[] based rep.
	 * 
...
//	}
	 ...
	 */
	protected void init(DatabaseImpl db, byte[] identifierKey,
	 ...
	 * equality and comparison for INs is based on a combination of the node id
	 * and identify hash code. Note that this will still give a correct equality
	 * value for any comparisons outside the INList sorted set.
	 */
	...
	/**
	 * Returns the last logged LSN, or null if never logged. Is public for unit
	 * testing.
	 */
	 ...
	 * 
	 * @param key -
	 *            the new identifier key for this node.
	 */
		...
		if (entryLsnLongArray == null) {
			...
			if (fileOffset == -1) {
				...
				return DbLsn.makeLsn(
						(long) (baseFileNumber + getFileNumberOffset(offset)),
						fileOffset);
			}
		} else {
	...
	void setLsnElement(int idx, long value) {
		...
		if (baseFileNumber == -1) {
			...
			setFileNumberOffset(offset,
					(byte) (thisFileNumber - baseFileNumber));
	 ...
	 * entryLsnByteArray.length; if (currentBytes < bytesNeeded) { int newBytes =
	 * bytesNeeded + (GROWTH_INCREMENT * BYTES_PER_LSN_ENTRY); byte[] newArr =
	 * new byte[newBytes]; System.arraycopy(entryLsnByteArray, 0, newArr, 0,
	 * currentBytes); entryLsnByteArray = newArr; for (int i = currentBytes; i <
	 * newBytes; i += BYTES_PER_LSN_ENTRY) { setFileNumberOffset(i, (byte) 0);
	 * setFileOffset(i, -1); } } } else { int currentEntries =
	 * entryLsnLongArray.length; int idx = offset >> 2; if (currentEntries < idx +
	 * 1) { int newEntries = idx + GROWTH_INCREMENT; long[] newArr = new
	 * long[newEntries]; System.arraycopy(entryLsnLongArray, 0, newArr, 0,
	 * currentEntries); entryLsnLongArray = newArr; for (int i = currentEntries;
	...
	private boolean adjustFileNumbers(long newBaseFileNumber) {
		...
		for (int i = 0; i < entryLsnByteArray.length; i += BYTES_PER_LSN_ENTRY) {
			...
			long curEntryFileNumber = oldBaseFileNumber
					+ getFileNumberOffset(i);
			...
			if (newCurEntryFileNumberOffset > Byte.MAX_VALUE) {
				...
				for (int j = i - BYTES_PER_LSN_ENTRY; j >= 0; j -= BYTES_PER_LSN_ENTRY) {
					...
					setFileNumberOffset(j,
							(byte) (getFileNumberOffset(j) - undoOffset));
	 ...
	 * knownDeleted entry has been cleaned. Note that null can only be returned
	 * for a slot that could contain a deleted LN, not other node types and not
	 * a DupCountLN since DupCountLNs are never deleted. Null is also returned
	 * for a KnownDeleted slot with a NULL_LSN.
	 * 
	...
	public Node fetchTarget(int idx) throws DatabaseException {
		...
		if (entryTargets[idx] == null) {
			...
			if (lsn == DbLsn.NULL_LSN) {
				...
				/*
				 * Ignore a NULL_LSN (return null) if KnownDeleted is set. This
				 * is the remnant of an incomplete insertion -- see Tree.insert.
				 * [#13126]
				 */
			} else {
				try {
					...
					if (!isEntryKnownDeleted(idx)
						...
						throw new DatabaseException(makeFetchErrorMsg(LNFE
								.toString(), this, lsn, entryStates[idx]));
					}

					/*
					 * Ignore. Cleaner got to the log file, so just return null.
					 * It is safe to ignore a deleted file for a pendingDeleted
					 * entry because the cleaner will not clean files with
					 * active transactions.
					 */
				} catch (Exception e) {
					throw new DatabaseException(makeFetchErrorMsg(e.toString(),
							this, lsn, entryStates[idx]), e);
				}
			}
		}
	 ...
	 */
	public void setEntry(int idx, Node target, byte[] keyVal, long lsn,
			byte state) {
	...
	/**
	 * Update the idx'th entry of this node. This flavor is used when the target
	 * LN is being modified, by an operation like a delete or update. We don't
	 * have to check whether the LSN has been nulled or not, because we know an
	 * LSN existed before. Also, the modification of the target is done in the
	 * caller, so instead of passing in the old and new nodes, we pass in the
	 * old and new node sizes.
	 */
	// public void updateEntry(int idx, long lsn, long oldLNSize, long
	// newLNSize) {
	// updateMemorySize(oldLNSize, newLNSize);//M.B
	// setLsn(idx, lsn);
	// setDirty(true);
	// }
	/**
	 * Update the idx'th entry of this node. Only update the key if the new key
	 * is less than the existing key.
	 */
	 ...
	 * @param indicateIfDuplicate -
	 *            true if EXACT_MATCH should be or'd onto the return value if
	 *            key is already present in this node.
	 * @param exact -
	 *            true if an exact match must be found.
	 * @return offset for the entry that has a key >= the arg. 0 if key is less
	...
	public int findEntry(byte[] key, boolean indicateIfDuplicate, boolean exact) {
		 ...
		 * exact match or to indicate duplicates, then use the key[0] and forget
		 * about the special entry zero comparison.
		 */
		boolean entryZeroSpecialCompare = entryZeroKeyComparesLow() && !exact
				&& !indicateIfDuplicate;
		 ...
		 * or return entry for which arg key is > entry key.
		 */
		if (exact) {
			return -1;
		} else {
	 ...
	 * INSERT_SUCCESS or EXACT_MATCH depending on whether the entry was inserted
	 * or it was a duplicate, resp.
	 * 
	 ...
	 *         duplicate in the IN |'d with EXACT_MATCH if the entry was found
	 *         to be a duplicate.
	 * 
	 * @throws InconsistentNodeException
	 *             if the node is full (it should have been split earlier).
	...
	public int insertEntry1(ChildReference entry) throws DatabaseException {
		...
		if (nEntries < entryTargets.length) {
			...
			throw new InconsistentNodeException("Node " + getNodeId()
	 ...
	 * 
	 * @param key
	 *            The key of the reference to delete from the IN.
	 * 
	 * @param maybeValidate
	 *            true if assert validation should occur prior to delete. Set
	 *            this to false during recovery.
	 * 
	...
	boolean deleteEntry(byte[] key, boolean maybeValidate)
		...
		if (nEntries == 0) {
			return false; // caller should put this node on the IN cleaner
			// list
		}
	 ...
	 * 
	 * @param index
	 *            The index of the entry to delete from the IN.
	 * 
	 * @param maybeValidate
	 ...
	 * 
	 * While we could latch couple down the tree, rather than hold latches as we
	...
	protected void splitInternal(IN parent, int childIndex, int maxEntries,
			int splitIndex) throws DatabaseException {
		...
		hook_splitInternal_work(newSibling, parent, childIndex, maxEntries,
				splitIndex, high, low, newIdKey, parentLsn);
	}

	private void hook_splitInternal_work(IN newSibling, IN parent,
			int childIndex, int maxEntries, int splitIndex, int high, int low,
			byte[] newIdKey, long parentLsn) throws DatabaseException {

		hookr_splitInternal_work2(newSibling, parent, childIndex, maxEntries,
				splitIndex, high, low, newIdKey, parentLsn);
		...
		// traceSplit(Level.FINE, parent, newSibling, parentLsn, myNewLsn,
		// newSiblingLsn, splitIndex, idKeyIndex, childIndex);
	}

	private void hookr_splitInternal_work2(IN newSibling, IN parent,
			int childIndex, int maxEntries, int splitIndex, int high, int low,
			byte[] newIdKey, long parentLsn) throws DatabaseException {
			...
			if (isEntryPendingDeleted(i)) {
				if (!deletedEntrySeen) {
					...
					binRef = new BINReference(newSibling.getNodeId(),
							databaseImpl.getId(), newIdKey);
				}
				binRef.addDeletedKey(new Key(thisKey));
			}
			newSibling.setEntry(toIdx++, entryTargets[i], thisKey, getLsn(i),
					entryStates[i]);
			clearEntry(i);
		}
		 ...
		 * have to adjust one pointer and add a new one, or we have to just add
		 * a new pointer to the new sibling.
		 * 
		 * Note that we must use the provisional form of logging because all
		 * three log entries must be read atomically. The parent must get logged
		 * last, as all referred-to children must preceed it. Provisional
		 * entries guarantee that all three are processed as a unit. Recovery
		 * skips provisional entries, so the changed children are only used if
		 * the parent makes it out to the log.
		 */
		EnvironmentImpl env = databaseImpl.getDbEnvironment();
		...
		/*
		 * When we update the parent entry, we use updateEntryCompareKey so that
		 * we don't replace the parent's key that points at 'this' with a key
		 * that is > than the existing one. Replacing the parent's key with
		 * something > would effectively render a piece of the subtree
		 * inaccessible. So only replace the parent key with something <= the
		 * existing one. See tree/SplitTest.java for more details on the
		 * scenario.
		 */
		if (low == 0) {

			/*
			 * Change the original entry to point to the new child and add an
			 * entry to point to the newly logged version of this existing
			 * child.
			 */
			if (childIndex == 0) {
			...
			boolean insertOk = parent.insertEntry(new ChildReference(this,
					entryKeyVals[0], myNewLsn));
			assert insertOk;
		} else {
			...
			if (childIndex == 0) {
				 ...
				 */
				parent.updateEntryCompareKey(childIndex, this, myNewLsn,
						entryKeyVals[0]);
			} else {
				parent.updateEntry(childIndex, this, myNewLsn);
			}
			boolean insertOk = parent.insertEntry(new ChildReference(
		 ...
		 * checkpointed. Other parents who are not roots are logged as part of
		 * the propagation of splits upwards.
		 */
		if (parent.isRoot()) {
		...
		/*
		 * Update size. newSibling and parent are correct, but this IN has had
		 * its entries shifted and is not correct.
		 */
	}
	 ...
	 * 
	 * @param index -
	 *            The position to start shifting from.
	 */
	 ...
	 * 
	 * @param byHowMuch -
	 *            The number of entries to remove from the left side of the
	 *            entries array.
	 */
	 ...
	 * already held before this is called. Also count removed nodes as obsolete.
	 */
	void accountForSubtreeRemoval(INList inList, UtilizationTracker tracker)
			throws DatabaseException {

		if (nEntries > 1) {
			throw new DatabaseException("Found non-deletable IN " + getNodeId()
		...
		/*
		 * Remove your children. They should already be resident. (LNs know how
		 * to stop.)
		 */
		for (int i = 0; i < nEntries; i++) {
	 ...
	 * have to latch the target to look at them. Also, this node is latched upon
	 * entry.
	 * 
	 * @param doFetch
	 *            If true, fetch the child in the pursuit of this search. If
	 *            false, give up if the child is not resident. In that case, we
	 *            have a potential ancestor, but are not sure if this is the
	 *            parent.
	 */
	void findParent(Tree.SearchType searchType, long targetNodeId,
			boolean targetContainsDuplicates, boolean targetIsRoot,
			byte[] targetMainTreeKey, byte[] targetDupTreeKey,
			SearchResult result, boolean requireExactMatch,
			boolean updateGeneration, int targetLevel, List trackingList,
			boolean doFetch) throws DatabaseException {
		...
		if (getNEntries() == 0) {
			...
			if (child.isSoughtNode(targetNodeId, updateGeneration)) {
				 ...
				 * child.
				 */
				descendOnParentSearch(result, targetContainsDuplicates,
						targetIsRoot, targetNodeId, child, requireExactMatch);

				/* If we're tracking, save the lsn and node id */
				if (trackingList != null) {
					if ((result.parent != this) && (result.parent != null)) {
						trackingList.add(new TrackingInfo(childLsn, child
								.getNodeId()));
					}
				}
				return;
	...
	public void hook_releaseLatch() throws DatabaseException {
	}

	public boolean hook_latchNoWait(boolean b) throws DatabaseException {
		return true;
	}
	...
	/*
	 * @return true if this IN is the child of the search chain. Note that if
	 * this returns false, the child remains latched.
	 */
	...
	public long log(LogManager logManager) throws DatabaseException {
		...
		return logInternal(logManager, false, // allowDeltas
				false, // isProvisional
	 ...
	 * Log this IN and clear the dirty flag.
	 */
	public long log(LogManager logManager, boolean allowDeltas,
			boolean proactiveMigration) throws DatabaseException {

		return logInternal(logManager, allowDeltas, false, // isProvisional
				proactiveMigration, null); // parent
	}
	...
	public long log(LogManager logManager, boolean allowDeltas,
		...
		return logInternal(logManager, allowDeltas, isProvisional,
				proactiveMigration, parent);
	}
	...
	protected long logInternal(LogManager logManager, boolean allowDeltas,
		 ...
		 * any provisionally logged descendants are immediately obsolete and can
		 * be flushed. If logging provisionally, the last version isn't obsolete
		 * until an ancestor is logged non-provisionally, so propagate obsolete
		 * lsns upwards.
		 */
		...
		if (isProvisional) {
			if (parent != null) {
				parent.trackProvisionalObsolete(this, lastFullVersion,
						DbLsn.NULL_LSN);
			}
		} else {
	...
	public int getLogSize() {
		...
		size += LogUtils.getByteArrayLogSize(identifierKey); // identifier
		// key
		...
		for (int i = 0; i < nEntries; i++) { // entries
			size += LogUtils.getByteArrayLogSize(entryKeyVals[i]) + // key
					(compactLsnsRep ? LogUtils.INT_BYTES : LogUtils
							.getLongLogSize()) + // LSN
					1; // state
		}
		return size;
		...
		for (int i = 0; i < nEntries; i++) {
			...
			if (compactLsnsRep) {
				...
				if (fileOffset == THREE_BYTE_NEGATIVE_ONE) {
					...
					lsn = DbLsn
							.makeLsn(storedBaseFileNumber + fileNumberOffset,
									fileOffset);
				}
			} else {
				/* LSNs in long form. */
	...
	public void dumpLog(StringBuffer sb, boolean verbose) {
		...
		if (verbose) {
			for (int i = 0; i < nEntries; i++) {
				...
				sb.append("\" pendingDeleted=\"").append(
	...
	public String dumpString(int nSpaces, boolean dumpTags) {
		...
		sb.append("<idkey>");
		sb
				.append(identifierKey == null ? "" : Key.dumpString(
						identifierKey, 0));
		sb.append("</idkey>");
		sb.append('\n');
		sb.append(TreeUtils.indent(nSpaces + 2));
	 ...
	 */
	/*
	 * private void traceSplit(Level level, IN parent, IN newSibling, long
	 * parentLsn, long myNewLsn, long newSiblingLsn, int splitIndex, int
	 * idKeyIndex, int childIndex) { Logger logger =
	 * databaseImpl.getDbEnvironment().getLogger(); if
	 * (logger.isLoggable(level)) { StringBuffer sb = new StringBuffer();
	 * sb.append(TRACE_SPLIT); sb.append(" parent=");
	 * sb.append(parent.getNodeId()); sb.append(" child=");
	 * sb.append(getNodeId()); sb.append(" newSibling=");
	 * sb.append(newSibling.getNodeId()); sb.append(" parentLsn = ");
	 * sb.append(DbLsn.getNoFormatString(parentLsn)); sb.append(" childLsn = ");
	 * sb.append(DbLsn.getNoFormatString(myNewLsn)); sb.append(" newSiblingLsn =
	 * "); sb.append(DbLsn.getNoFormatString(newSiblingLsn)); sb.append("
	 * splitIdx="); sb.append(splitIndex); sb.append(" idKeyIdx=");
	 * sb.append(idKeyIndex); sb.append(" childIdx="); sb.append(childIndex);
	 ...
	 */
	/*
	 * private void traceDelete(Level level, int index) { Logger logger =
	 * databaseImpl.getDbEnvironment().getLogger(); if
	 * (logger.isLoggable(level)) { StringBuffer sb = new StringBuffer();
	 * sb.append(TRACE_DELETE); sb.append(" in=").append(getNodeId());
	 * sb.append(" index="); sb.append(index); logger.log(level, sb.toString()); } }
	 */
}
