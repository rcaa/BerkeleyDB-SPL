 ...
 *
 * $Id: Key.java,v 1.1.6.1 2006/07/28 09:01:58 ckaestne Exp $
 */
...
public final class Key implements Comparable {
	...
	public static byte[] makeKey(DatabaseEntry dbt) {
		...
		if (entryKey == null) {
			...
			System.arraycopy(entryKey, dbt.getOffset(), newKey, 0, dbt
					.getSize());
			return newKey;
		}
	}
	...
	public static String dumpString(byte[] key, int nspaces) {
		...
		/**
		 * uncomment for hex formatting for (int i = 0 ; i < key.length; i++) {
