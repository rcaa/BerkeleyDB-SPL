 ...
 *
 * $Id: Tree.java,v 1.4.6.2.2.4 2006/10/20 19:58:05 ckaestne Exp $
 */
...
public final class Tree implements LogWritable, LogReadable {
	 ...
	 */
	public void setRoot(ChildReference newRoot/* Lck, boolean notLatched */) {
		root = newRoot;
	}
	...
	/*
	 * Class that overrides fetchTarget() so that if the rootLatch is not held
	 * exclusively when the root is fetched, we upgrade it to exclusive.
	 */
	...
	// Lck introduced!
	public IN withRoot(WithRootLatched wrl)
	...
	public void delete(byte[] idKey, UtilizationTracker tracker)
		 ...
		 * deletable subtree latched as we descend so we can log the IN deletion
		 * and cascade the logging up the tree. The latched nodes are kept in
		 * order in the nodeLadder.
		 */
		ArrayList nodeLadder = new ArrayList();

		IN rootIN = null;
		boolean rootNeedsUpdating = false;
		// Lck rootLatch.acquireExclusive();
		// try {
		if (root == null) {
		...
		if (nodeLadder.size() == 0) {
			...
			/*
			 * The root is the top of the deletable subtree. Delete the whole
			 * tree if the purge root je property is set. In general, there's no
			 * reason to delete this last IN->...IN->BIN subtree since we're
			 * likely to to add more nodes to this tree again. Deleting the
			 * subtree also adds to the space used by the log since a MapLN
			 * needs to be written when the root is nulled, and a MapLN, IN
			 * (root), BIN needs to be written when the root is recreated.
			 * 
			 * Consider a queue application which frequently inserts and deletes
			 * entries and often times leaves the tree empty, but will insert
			 * new records again.
			 * 
			 * An optimization might be to prune the multiple IN path to the
			 * last BIN (if it even exists) to just a root IN pointing to the
			 * single BIN, but this doesn't feel like it's worth the trouble
			 * since the extra depth doesn't matter all that much.
			 */

			subtreeRootIN = hook_updateRoot(rootIN,tracker);
		...
		if (subtreeRootIN != null) {
			...
			if (rootNeedsUpdating) {
				...
				// TODO RecoveryManager.traceRootDeletion(Level.FINE, database);
			}

			/*
			 * Count obsolete nodes after logging the delete. We can do this
			 * without having the nodes of the subtree latched because the
	...
	// Lck private void releaseNodeLadderLatches(ArrayList nodeLadder)
	// throws DatabaseException {
	//
	// /*
	// * Clear any latches left in the node ladder. Release from the bottom
	// * up.
	// */
	...
	private IN hook_updateRoot(IN rootIN, UtilizationTracker tracker) throws DatabaseException {
	...
	public void deleteDup(byte[] idKey, byte[] mainKey,
		...
		IN in = search(mainKey, SearchType.NORMAL, -1, null, false /* updateGeneration */);

		IN deletedSubtreeRoot = null;
		// try {
		// Lck assert in.isLatchOwner();
	...
	private IN deleteDupSubtree(byte[] idKey, BIN bin, int index)
		...
		try {
			...
			/*
			 * We don't release the latch on bin before we search the duplicate
			 * tree below because we might be deleting the whole subtree from
			 * the IN and we want to keep it latched until we know.
			 */
			...
			if (nodeLadder.size() == 0) {
				...
				/*
				 * Cascade updates upward, including writing the root DIN and
				 * parent BIN.
				 */
			...
			// Lck releaseNodeLadderLatches(nodeLadder);

			if (dupCountLNLocked) {
				locker.releaseLock(dcl.getNodeId());
			}

			// Lck if (duplicateRoot != null) {
	...
	public IN getFirstNode() throws DatabaseException {
		...
		return search(null, SearchType.LEFT, -1, null, true /* updateGeneration */);
	}
	...
	public IN getLastNode() throws DatabaseException {
		...
		return search(null, SearchType.RIGHT, -1, null, true /* updateGeneration */);
	}
	...
	public DBIN getFirstNode(DIN dupRoot) throws DatabaseException {
		...
		// Lck assert dupRoot.isLatchOwner();

		IN ret = searchSubTree(dupRoot, null, SearchType.LEFT, -1, null, true /* updateGeneration */);
		return (DBIN) ret;
	}
	...
	public DBIN getLastNode(DIN dupRoot) throws DatabaseException {
		...
		IN ret = searchSubTree(dupRoot, null, SearchType.RIGHT, -1, null, true /* updateGeneration */);
		return (DBIN) ret;
	}
	...
	public SearchResult getParentINForChildIN(IN child,
		...
		// Lck child.releaseLatch();

		return getParentINForChildIN(child.getNodeId(), child
				.containsDuplicates(), isRoot, mainTreeKey, dupTreeKey,
		...
		if (rootIN != null) {
			...
			try {
				while (result.keepSearching) {
					...
					/*
					 * [12736] Prune away oldBin. Assert has intentional side
					 * effect.
					 */
				...
				// Lck potentialParent.releaseLatchIfOwner();
	...
	public boolean getParentBINForChildLN(TreeLocation location,
		...
		try {
			...
			if (searchResult != null) {
				// Lck searchResult.releaseLatchIfOwner();
		...
		if (match) {
			...
			/*
			 * A BIN parent was found and a slot matches the key. See if we have
			 * to search further into what may be a dup tree.
			 */
			if (!location.bin.isEntryKnownDeleted(location.index)) {

				/*
				 * If this database doesn't support duplicates, no point in
				 * incurring the potentially large cost of fetching in the child
				 * to check for dup trees. In the future, we could optimize
				 * further by storing state per slot as to whether a dup tree
				 * hangs below.
				 */
					...
					if (childNode == null) {
						...
						if (childNode.containsDuplicates()) {
							if (dupKey == null) {
								...
								/*
								 * We are at a dup tree but our target LN has no
								 * dup key because it's a deleted LN. We've
								...
								return searchDupTreeForDBIN(location, dupKey,
										(DIN) childNode, ln,
										findDeletedEntries, indicateIfExact,
										exactSearch, splitsAllowed,
										updateGeneration);
							}
						}
					}
					// } catch (DatabaseException e) {
					// Lck location.bin.releaseLatchIfOwner();
					// throw e;
					// }
				}
			}
	...
	private boolean searchDupTreeByNodeId(TreeLocation location,
		...
		if (searchDupTree) {
			...
			/*
			 * This is called from undo() so this LN can just be ignored.
			 */
			return false;
		}
	}
	...
	private boolean searchDupTreeForDupCountLNParent(TreeLocation location,
		...
		if (childNode instanceof DIN) {
			 ...
			 * tree, then the key now refers to a single datum. This can happen
			 * when all dups for a key are deleted, the compressor runs, and
			 * then a single datum is inserted. [#10597]
			 */
			return false;
		}
	}
	...
	private boolean searchDupTreeForDBIN(TreeLocation location, byte[] dupKey,
		...
		if (splitsAllowed) {
			try {
				...
				/*
				 * Shouldn't happen; the only caller of this method which allows
				 * splits is from recovery, which is single threaded.
				 */
				throw new DatabaseException(e);
			}
		} else {
			location.bin = (BIN) searchSubTree(dupRoot, location.lnKey,
					SearchType.NORMAL, ln.getNodeId(), null, updateGeneration);
		}
	...
	private BIN getNextBinInternal(boolean traverseWithinDupTree, BIN bin,
		 ...
		 * most key (for a backward progressing cursor) as the idkey. The reason
		 * is that the BIN may get split while finding the next BIN so it's not
		 * safe to take the BIN's identifierKey entry. If the BIN gets splits,
		...
		// Lck assert LatchSupport.countLatchesHeld() == 1 : LatchSupport
		// .latchesHeldToString();
		...
		while (true) {
			...
			if (!traverseWithinDupTree) {
				...
				result = getParentINForChildIN(next,
						true /* requireExactMatch */, true /* updateGeneration */);
				if (result.exactParentFound) {
					parent = result.parent;
				} else {
					/* We've reached the root of the tree. */
					// Lck assert (LatchSupport.countLatchesHeld() == 0) :
					// LatchSupport
					// .latchesHeldToString();
					return null;
				}
			...
			/*
			 * Figure out which entry we are in the parent. Add (subtract) 1 to
			 * move to the next (previous) one and check that we're still
			...
			if (moreEntriesThisBin) {
				...
				// Lck assert (LatchSupport.countLatchesHeld() == 2) :
				// LatchSupport
				// .latchesHeldToString();

				if (nextIN instanceof BIN) {
					/* We landed at a leaf (i.e. a BIN). */
					hook_foundLeafBIN(nextIN);
					// Lck parent.releaseLatch();
					...
					/*
					 * We landed at an IN. Descend down to the appropriate leaf
					 * (i.e. BIN) node.
					 */
					IN ret = searchSubTree(nextIN, null,
							(forward ? SearchType.LEFT : SearchType.RIGHT), -1,
							null, true /* updateGeneration */);
					// Lck parent.releaseLatch();

					// Lck assert LatchSupport.countLatchesHeld() == 1 :
					// LatchSupport
					// .latchesHeldToString();
	...
	private void hook_foundLeafBIN(IN nextIN) {
		// TODO Auto-generated method stub
	...
	private void splitRoot() throws DatabaseException {
		...
		try {
			...
			boolean insertOk = newRoot.insertEntry(new ChildReference(curRoot,
					rootIdKey, curRootLsn));
			assert insertOk;
		...
		// TODO traceSplitRoot(Level.FINE, TRACE_ROOT_SPLIT, newRoot, logLsn,
		// curRoot,
		// curRootLsn);
	}
			...
			while (true) {
				if (root != null) {
					...
					if (rootIN.needsSplitting()) {
						 ...
						 * splits again between latch release and DbTree.db
						 * lock, no problem. The latest root will still get
						 * written out.
						 */
						// Lck rootLatch.release();
						rootLatched = false;
	...
	public IN searchSubTree(IN parent, byte[] key, SearchType searchType,
		...
		// Lck assert parent.isLatchOwner();
		...
		try {
			do {
				hook_beforeLoopChildren(parent);
				...
				// Lck child.latch(updateGeneration);
	...
	private void hook_beforeLoopChildren(IN parent) {
		// TODO Auto-generated method stub
	 ...
	 * A private void hook_beforeLoopChildren(IN parent) {
		// TODO Auto-generated method stub
		
	}

deletable subtree is a branch where each IN has one child, and the
	...
	public void searchDeletableSubTree(IN parent, byte[] key,
		...
		/*
		 * See if there is a reason we can't delete this BIN -- i.e. new items
		 * have been inserted, or a cursor exists on it.
		 */
		if ((child != null) && (child instanceof BIN)) {
			...
			/*
			 * This case can happen if we are keeping a BIN on an empty cursor
			 * as we traverse.
			 */
		...
		if (lowestMultipleEntryIN != null) {
			/*
			 * Release all nodes up to the pair that holds the detach point. We
			 * won't be needing those nodes, since they'll be pruned and won't
			 * need to be updated.
			 */
			ListIterator iter = nodeLadder.listIterator(nodeLadder.size());
			while (iter.hasPrevious()) {
				...
				if (info.parent == lowestMultipleEntryIN) {
					...
					// Lck info.child.releaseLatch();
					iter.remove();
				}
			}
		} else {
			/*
			 * We actually have to prune off the entire tree. Release all
			 * latches, and clear the node ladder.
			 */
			// Lck releaseNodeLadderLatches(nodeLadder);
			nodeLadder.clear();
		}
	...
	private void forceSplit(IN parent, byte[] key) throws DatabaseException,
		...
		// Lck originalParent.latch();

		/*
		 * Another thread may have crept in and - used the last free slot in the
		 * parent, making it impossible to correctly progagate the split. -
		 * actually split the root, in which case we may be looking at the wrong
		 * subtree for this search. If so, throw and retry from above. SR
		 * [#11144]
		 */
		...
		/*
		 * Search downward to the BIN level, saving the information needed to do
		 * a split if necessary.
		 */
		do {
			if (parent.getNEntries() == 0) {
				/* No more children, can't descend anymore. */
			...
			/*
			 * Get the child node that matches. We only need to work on nodes in
			 * residence.
			 */
		...
		/*
		 * Process the accumulated nodes from the bottom up. Split each node if
		 * required. If the node should not split, we check if there have been
		 * any splits on the ladder yet. If there are none, we merely release
		 * the node, since there is no update. If splits have started, we need
		 * to propagate new LSNs upward, so we log the node and update its
		 * parent.
		 * 
		...
		while (iter.hasPrevious()) {
			...
			if (child.needsSplitting()) {
				...
				if (allLeftSideDescent || allRightSideDescent) {
					child.splitSpecial(parent, index, maxEntriesPerNode, key,
							allLeftSideDescent);
				} else {
				 ...
				 * reference. Now the MapLN is logically dirty, but the change
				 * hasn't been logged. Set the rootIN to be dirty again, to
				 * force flushing the rootIN and mapLN in the next checkpoint.
				 * Be sure to flush the MapLN if we ever evict the root.
				 */
				if (parent.isDbRoot()) {
				...
				if (startedSplits) {
					 ...
					 * logged by the split call. We just need to propagate the
					 * logging upwards. If this child is just a link in the
					 * chain upwards, log it.
					 */
		...
		// child.releaseLatchIfOwner();
		// }
		// Lck originalParent.releaseLatchIfOwner();
		// }
		//
		// /*
		...
		// while (iter.hasPrevious()) {
		// SplitInfo info = (SplitInfo) iter.previous();
		// // Lck info.child.releaseLatchIfOwner();
		// }
		// }

		// Lck if (isRootLatched) {
		// rootLatch.release();
		// }
		// }
	}
	...
	public IN getRootIN(boolean updateGeneration) throws DatabaseException {
		...
		if (root != null) {
			rootIN = (IN) root.fetchTarget(database, null);
			// Lck rootIN.latch(updateGeneration);
		}
		return rootIN;
		// Lck } finally {
		// rootLatch.release();
	...
	public boolean insert(LN ln, byte[] key, boolean allowDuplicates,
		...
		try {
			 ...
			 * BIN correctly here so that adjustCursorsForInsert doesn't blow an
			 * assertion. We'll finish the job by setting the index below.
			 */
			cursor.setBIN(bin);
			...
			if ((index & IN.INSERT_SUCCESS) != 0) {
				...
				try {
					...
					if (newLsn == DbLsn.NULL_LSN) {
						 ...
						 * exception during logging. The BIN entry will contain
						 * a NULL_LSN. To prevent an exception during a fetch,
						 * we set the KnownDeleted flag. We do not call
						 * BIN.deleteEntry because cursors will not be adjusted.
						 * We do not add this entry to the compressor queue to
						 * avoid complexity (this is rare). [13126, 12605,
						 * 11271]
						 */
				...
				// TODO traceInsert(Level.FINER, env, bin, ln, newLsn, index);
				return true;
			} else {
				...
				if (!isDup) {
					if (currentLN == null) {
						...
						if (cursor.getDupBIN() != null) {
							 ...
							 * from this dup tree entry. This is a rare case so
							 * performance is not an issue.
							 */
							cursor.clearDupBIN(/*Lck true /* alreadyLatched */);
							isDup = true;
				...
				if (isDeleted) {
					...
					/*
					 * Set the abort LSN to that of the lock held on the current
					 * LN, if the current LN was previously locked by this txn.
					 * This is needed when we change the node ID of this slot.
					 * 
					 * If reusing a slot with a deleted LN deleted in a prior
					 * transaction (the LockGrantType is NEW or UPGRADE), always
					 * set abortKnownDeleted=true. It may be that the existing
					 * slot is PENDING_DELETED, but we restore to KNOWN_DELETED
					...
					// TODO traceInsert(Level.FINER, env, bin, ln, newLsn,
					// index);
					return true;
				} else {

					/*
					 * Attempt to insert a duplicate in an exception dup tree or
					 * create a dup tree if none exists.
					 */
	...
	private boolean insertDuplicate(byte[] key, BIN bin, LN newLN,
		...
		if (n instanceof DIN) {
			...
			/*
			 * Do not proceed if duplicates are not allowed and there are one or
			 * more duplicates already present. Note that if the dup count is
			 * zero, we allow the insert.
			 */
			if (!allowDuplicates) {
			...
			/*
			 * Split the dup root if necessary. The dup root may have changed
			 * during locking above or by the split, so refetch it. In either
			 * case it will be latched.
			 */
			maybeSplitDuplicateRoot(bin, index);
			dupRoot = (DIN) bin.fetchTarget(index);
			/*
			 * Search the duplicate tree for the right place to insert this new
			 * record. Releases the latch on duplicateRoot. If the duplicateRoot
			 * got logged as a result of some splitting, update the BIN's LSN
			 * information. The SortedLSNTreeWalker relies on accurate LSNs in
			 * the in-memory tree.
			 */
			byte[] newLNKey = newLN.getData();
			long previousLsn = dupRoot.getLastFullVersion();
			try {
				dupBin = (DBIN) searchSubTreeSplitsAllowed(dupRoot, newLNKey,
						-1, true /* updateGeneration */);
			} catch (SplitRequiredException e) {

				/*
				 * Shouldn't happen -- we have the DIN in the root of the dup
				 * tree latched during this insert, so there should be no
				 * possibility of being unable to insert a new entry into the
				 * DIN root of the dup tree.
				 */
				throw new DatabaseException(e);
			}
...
//			Lck cursor.releaseBIN();
			...
			/*
			 * Try to insert a new reference object. If successful, we'll log
			...
			if ((dupIndex & IN.INSERT_SUCCESS) != 0) {
				...
				// TODO traceInsertDuplicate(Level.FINER, database
				// .getDbEnvironment(), dupBin, newLN, newLsn, binNid);
				successfulInsert = true;
			} else {

				/*
				 * The insert was not successful. Either this is a duplicate
				 * duplicate or there is an existing entry but that entry is
				 * deleted.
				 */
				...
				if (isDeleted) {
					...
					// TODO traceInsertDuplicate(Level.FINER, database
					// .getDbEnvironment(), dupBin, newLN, newLsn,
					// binNid);
					successfulInsert = true;
				} else {
					/* Duplicate duplicate. */
			...
			/*
			 * To avoid latching out of order (up the tree), release the DBIN
			 * latch before latching the BIN and dup root.
			 */
			// Lck dupBin.releaseLatch();
			dupBin = null;

			if (successfulInsert) {
//				Lck cursor.latchBIN();
				dupRoot = cursor.getDupRoot();
//				Lck cursor.releaseBIN();
				dupRoot.incrementDuplicateCount(dclLockResult, key, cursor
						.getLocker(), true /* increment */);
			}
			// Lck } finally {
			// if (dupBin != null) {
			...
			/*
			 * There is no duplicate tree yet. The existing LN is guaranteed to
			 * be non-deleted, so to insert we must create a dup tree.
			 */
			if (!allowDuplicates) {
				return false;
			}
	...
	private boolean maybeSplitDuplicateRoot(BIN bin, int index)
		...
		if (curRoot.needsSplitting()) {
			...
			DIN newRoot = new DIN(database, rootIdKey,
					maxDupTreeEntriesPerNode, curRoot.getDupKey(), curRoot
							.getDupCountLNRef(), curRoot.getLevel() + 1);
			...
			/*
			 * Make the new root DIN point to the old root DIN, and then log. We
			 * should be able to insert into the root because the root is newly
			 * created.
			 */
			try {
				curRootLsn = curRoot.logProvisional(logManager, newRoot);
			...
			// curRoot.releaseLatch();
			// }
			// TODO traceSplitRoot(Level.FINE, TRACE_DUP_ROOT_SPLIT, newRoot,
			// logLsn,
			// curRoot, curRootLsn);
			return true;
		} else {
	...
	private DIN createDuplicateTree(byte[] key, LogManager logManager,
		...
		boolean keysEqual = Key.compareKeys(newLNKey, existingKey, database
				.getDuplicateComparator()) == 0;
		if (keysEqual) {
			return null;
		}
		 ...
		 * 
		 * Once we create a dup tree, we don't revert back to the LN. Create a
		 * DupCountLN to hold the count for this dup tree. Since we don't roll
		 * back the internal nodes of a duplicate tree, we need to create a
		 * pre-transaction version of the DupCountLN. This version must hold a
		 * count of either 0 or 1, depending on whether the current transaction
		 * created the exising lN or not. If the former, the count must roll
		 * back to 0, if the latter, the count must roll back to 1.
		 * 
		 ...
		 * DIN. This works:
		 * 
		 * Entry 1: (provisional) DupCountLN (first version) Entry 2:
		 * (provisional) DupBIN Entry 3: DIN Entry 4: DupCountLN (second
		 * version, incorporating the new count. This can't be provisional
		 * because we need to possibly roll it back.) Entry 5: new LN. See [SR
		 * #10203] for a description of the bug that existed before this change.
		 */
		...
		long firstDupCountLNLsn = dupCountLN.logProvisional(env, database
				.getId(), key, DbLsn.NULL_LSN);

		/* Make the duplicate root and DBIN. */
		dupRoot = new DIN(database, existingKey, // idkey
				maxDupTreeEntriesPerNode, key, // dup key
				new ChildReference(dupCountLN, key, firstDupCountLNLsn), 2); // level
		// Lck dupRoot.latch();
		dupRoot.setIsRoot(true);

		dupBin = new DBIN(database, existingKey, // idkey
				maxDupTreeEntriesPerNode, key, // dup key
				1); // level
		// Lck dupBin.latch();
		...
		dupRoot.setEntry(0, dupBin, dupBin.getKey(0), dbinLsn, dupBin
				.getState(0));
		...
		/*
		 * Now that the DIN is logged, we've created a duplicate tree that holds
		 * the single, preexisting LN. We can safely create the non provisional
		 * LNs that pertain to this insert -- the new LN and the new DupCountLN.
		 * 
		...
		LockResult lockResult = locker.lock(dupCountLN.getNodeId(),
				LockType.WRITE, false /* noWait */, database);
		...
		long newLsn = newLN.log(env, database.getId(), key, DbLsn.NULL_LSN,
				locker);
		int dupIndex = dupBin.insertEntry1(new ChildReference(newLN, newLNKey,
				newLsn));
		dupIndex &= ~IN.INSERT_SUCCESS;
		cursor.updateDBin(dupBin, dupIndex);

		/*
		 * Adjust any cursors positioned on the mutated BIN entry to point to
		 * the DBIN at the location of the entry we moved there. The index of
		 * the moved entry is 1 or 0, the XOR of the index of the new entry.
		 */
		 ...
		 * of the existing LN. Clear the MIGRATE flag since it applies only to
		 * the original LN.
		 */
		bin.updateEntry(index, dupRoot, dinLsn);
		bin.setMigrate(index, false);

		// TODO traceMutate(Level.FINE, bin, existingLN, newLN, newLsn,
		// dupCountLN,
		// dupCountLsn, dupRoot, dinLsn, dupBin, dbinLsn);
		// } catch (DatabaseException e) {

		/*
		 * Strictly speaking, not necessary to release latches, because if we
		 * fail to log the entries, we just throw them away, but our unit tests
		 * check for 0 latches held in the event of a logging error.
		 */
		// Lck dupBin.releaseLatchIfOwner();
		// Lck dupRoot.releaseLatchIfOwner();
		// throw e;
		// }
		return dupRoot;
	}
...
//		Lck bin = cursor.latchBIN();
		bin=cursor.getBIN();
		if (bin != null) {
			if (!bin.needsSplitting() && bin.isKeyInBounds(key)) {
				return bin;
				// } else {
				// Lck bin.releaseLatch();
			}
		}

//		Lck boolean rootLatchIsHeld = false;
		// Lck try {
		long logLsn;

		/*
		 * We may have to try several times because of a small timing window,
		 * explained below.
		 */
		while (true) {
			// Lck rootLatchIsHeld = true;
			// Lck rootLatch.acquireShared();
			if (root == null) {
				// Lck rootLatch.release();
				// Lck rootLatch.acquireExclusive();
				if (root != null) {
					// Lck rootLatch.release();
					// Lck rootLatchIsHeld = false;
					continue;
				}

				/*
				 * This is an empty tree, either because it's brand new tree or
				 * because everything in it was deleted. Create an IN and a BIN.
				 * We could latch the rootIN here, but there's no reason to
				 * since we're just creating the initial tree and we have the
				 * rootLatch held. Log the nodes as soon as they're created, but
				 * remember that referred-to children must come before any
				...
				/*
				 * Log the root right away. Leave the root dirty, because the
				 * MapLN is not being updated, and we want to avoid this
				 * scenario from [#13897], where the LN has no possible parent.
				 * provisional BIN root IN checkpoint start LN is logged
				 * checkpoint end BIN is dirtied, but is not part of checkpoint
				 */

				IN rootIN = new IN(database, key, maxMainTreeEntriesPerNode, 2);
				rootIN.setIsRoot(true);

				boolean insertOk = rootIN.insertEntry(new ChildReference(bin,
						key, logLsn));
				assert insertOk;
...
//				Lck rootLatchIsHeld = false;
				...
				/*
				 * There's a tree here, so search for where we should insert.
				 * However, note that a window exists after we release the root
				 * latch. We release the latch because the search method expects
				 * to take the latch. After the release and before search, the
				 * INCompressor may come in and delete the entire tree, so
				 * search may return with a null.
				 */
				IN in = searchSplitsAllowed(key, -1, true /* updateGeneration */);
				if (in == null) {
	...
	private void accountForSubtreeRemoval(INList inList, IN subtreeRoot,
		...
		// Lck inList.latchMajor();
		// try {
		subtreeRoot.accountForSubtreeRemoval(inList, tracker);
		// } finally {
		// Lck inList.releaseMajorLatch();
		// }

		// refined trace Tracer.trace(Level.FINE,
		// database.getDbEnvironment(),"SubtreeRemoval: subtreeRoot = "
	...
	// * to construct the message if the level is not enabled.
	// */
	// private void traceMutate(Level level, BIN theBin, LN existingLn, LN
	// newLn,
