 ...
 *
 * $Id: ChildReference.java,v 1.1.6.1.2.3 2006/10/25 17:21:16 ckaestne Exp $
 */
...
public class ChildReference implements LogWritable, LogReadable {
	 ...
	 * 
	 * @param database
	 *            The database that this ChildReference resides in.
	 * @param in
	 *            The IN that this ChildReference lives in. If the target is
			...
			if (lsn == DbLsn.NULL_LSN) {
				...
				try {
					...
					if (!isKnownDeleted() && !isPendingDeleted()) {
						throw new DatabaseException(IN.makeFetchErrorMsg(LNFE
								.toString(), in, lsn, state), LNFE);
					}
					/* Ignore. Cleaner got to it, so just return null. */
				} catch (Exception e) {
					throw new DatabaseException(IN.makeFetchErrorMsg(e
							.toString(), in, lsn, state), e);
				}
			}
		}
