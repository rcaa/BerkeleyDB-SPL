 ...
 *
 * $Id: DbDump.java,v 1.4.8.3 2006/10/23 21:07:31 ckaestne Exp $
 */
...
public class DbDump {
	...
	public DbDump(Environment env, String dbName, PrintStream outputFile,
			String outputDirectory, boolean formatUsingPrintable) {
		try {
	...
	protected void openEnv(boolean doRecovery) throws DatabaseException {
		...
		if (env == null) {
			...
			/* Don't run recovery. */
			envConfiguration.setConfigParam(EnvironmentParams.ENV_RECOVERY
					.getName(), doRecovery ? "true" : "false");
	...
	public void dump() throws IOException, DatabaseException {
		...
		Database db = env.openDatabase((OperationContext) null, dbName,
				dbConfig);
		dupSort = db.getConfig().getSortedDuplicates();

		printHeader(outputFile, dupSort, formatUsingPrintable);

		Cursor cursor = db.openCursor((OperationContext) null, null);
	...
	protected void printHeader(PrintStream o, boolean dupSort,
