 ...
 *
 * $Id: DbInternal.java,v 1.1.6.3 2006/07/28 17:00:05 ckaestne Exp $
 */
...
/**
 * For internal use only. It serves to shelter methods that must be public to be
 * used by other BDBJE packages but that are not part of the public api
...
public class DbInternal {
	 ...
	 */
	public static Cursor newCursor(DatabaseImpl dbImpl, Locker locker,
	 ...
	 */
	public static OperationStatus position(Cursor cursor, DatabaseEntry key,
	...
	public static OperationStatus retrieveNext(Cursor cursor,
			...
			GetMode getMode) throws DatabaseException {
	 ...
	 */
	public static boolean advanceCursor(Cursor cursor, DatabaseEntry key,
			DatabaseEntry data) {
	 ...
	 */
	public static EnvironmentMutableConfig cloneMutableConfig(
	...
	public static void databaseConfigValidate(DatabaseConfig config1,
			DatabaseConfig config2) throws DatabaseException {
	...
	/**
	 * Get an Environment only if the environment is already open. This will
	 * register this Enviroment in the EnvironmentImpl's reference count, but
	 * will not configure the environment.
	 * 
	...
	public static Environment getEnvironmentShell(File environmentHome) {
		...
		try {
			...
			/*
			 * Klockwork - ok the environment is not valid.
			 */
		}
		return env;
	}
