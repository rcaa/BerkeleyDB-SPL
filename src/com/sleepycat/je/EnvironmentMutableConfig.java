 ...
 *
 * $Id: EnvironmentMutableConfig.java,v 1.1.6.1.2.3 2006/10/25 17:21:33 ckaestne Exp $
 */
...
public class EnvironmentMutableConfig implements Cloneable {
	...
	/**
	 * Internal boolean that says whether or not to validate params. Setting it
	 * to false means that parameter value validatation won't be performed
	...
	/**
	 * Gets either the value stored in this configuration or the default value
	 * for this param.
	 */
	...
	/**
	 * Check that the immutable values in the environment config used to open an
	 * environment match those in the config object saved by the underlying
	...
	void checkImmutablePropsForEquality(EnvironmentMutableConfig passedConfig)
		...
		Iterator iter = EnvironmentParams.getSupportedParams().keySet().iterator();
		while (iter.hasNext()) {
			...
			if (!param.isMutable()) {
				...
				if ((paramVal != null) ? (!paramVal.equals(useParamVal))
						: (useParamVal != null)) {
					throw new IllegalArgumentException(paramName
							+ " is set to " + useParamVal
							+ " in the config parameter"
							+ " which is incompatible" + " with the value of "
							+ paramVal + " in the" + " underlying environment");
				}
			}
		}
	}
	...
	protected Object clone() throws CloneNotSupportedException {
		...
		EnvironmentMutableConfig copy = (EnvironmentMutableConfig) super
				.clone();
	...
	/**
	 * Fill in the properties calculated by the environment to the given config
	 * object.
	 */
