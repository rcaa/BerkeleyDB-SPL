 ...
 *
 * $Id: LNLogEntry.java,v 1.1.6.1 2006/07/28 09:02:35 ckaestne Exp $
 */
...
public class LNLogEntry implements LogEntry, LoggableObject, NodeLogEntry {
	...
	public LNLogEntry(LogEntryType entryType, LN ln, DatabaseId dbId,
			byte[] key, long abortLsn, boolean abortKnownDeleted, Txn txn) {
		...
		try {
			if (readFullItem) {
				...
				if (isTransactional) {
					...
					/*
					 * AbortLsn. If it was a marker LSN that was used to fill in
					 * a create, mark it null.
					 */
					abortLsn = LogUtils.readLong(entryBuffer);
					if (DbLsn.getFileNumber(abortLsn) == DbLsn
							.getFileNumber(DbLsn.NULL_LSN)) {
						abortLsn = DbLsn.NULL_LSN;
					}

					abortKnownDeleted = ((entryBuffer.get() & ABORT_KNOWN_DELETED_MASK) != 0) ? true
							: false;
				...
				/*
				 * Read node ID and position to end. We currently do not support
	...
	/**
	 * @see LoggableObject#marshallOutsideWriteLatch Ask the ln if it can be
	...
	public int getLogSize() {
		int size = ln.getLogSize() + dbId.getLogSize()
