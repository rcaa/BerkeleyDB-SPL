 ...
 *
 * $Id: FileReader.java,v 1.4.6.2.2.5 2007/03/01 23:15:11 ckaestne Exp $
 */
...
public abstract class FileReader {
	...
	/*
	 * ReadBufferFileNum, readBufferFileStart and readBufferFileEnd indicate how
	 * the read buffer maps to the file. For example, if the read buffer size is
	 * 100 and the read buffer was filled from file 9, starting at byte 100,
	 * then readBufferFileNum = 9 readBufferFileStart = 100 readBufferFileEnd =
	 * 200
	 */
	...
	protected boolean currentEntryCollectData;

	/*
	 * nextEntryOffset is used to set the currentEntryOffset after we've read an
	 * entry.
	 */
	...
	public FileReader(EnvironmentImpl env, int readBufferSize, boolean forward,
		...
		maxReadBufferSize = configManager
				.getInt(EnvironmentParams.LOG_ITERATOR_MAX_SIZE);
	...
	protected void initStartingPosition(long endOfFileLsn,
			Long ignoreSingleFileNumber) throws IOException, DatabaseException {

		eof = false;
		if (forward) {

			/*
			 * Start off at the startLsn. If that's null, start at the beginning
			 * of the log. If there are no log files, set eof.
			 */
			...
			/*
			 * Make the read buffer look like it's positioned off the end of the
			 * file. Initialize the first LSN we want to read. When traversing
			 * the log backwards, we always start at the very end.
			 */
			 ...
			 */
			if (DbLsn.getFileNumber(startLsn) == DbLsn
					.getFileNumber(endOfFileLsn)) {
	...
	public boolean readNextEntry() throws DatabaseException, IOException {
		...
		try {
			while ((!eof) && (!foundEntry)) {
				...
				// TODO turned local variable to member variable to that it can
				// be changed in the pointcut
			...
			eof = true;
			/* Report on error. */
			LogEntryType problemType = LogEntryType.findType(
					currentEntryTypeNum, currentEntryTypeVersion);
			// refined trace
			hook_traceReadNext(problemType,e);
	...
	private void hook_traceReadNext(LogEntryType problemType, DatabaseException e) {
	...
	protected boolean resyncReader(long nextGoodRecordPostCorruption,
			boolean dumpCorruptedBounds) throws DatabaseException, IOException {
	 ...
	 */
	private void getLogEntryInReadBuffer() throws IOException,
			DatabaseException, EOFException {

		/*
		 * If we're going forward, because we read every byte sequentially,
		 * we're always sure the read buffer is positioned at the right spot. If
		 * we go backwards, we need to jump the buffer position.
		 */
		if (!forward) {
			...
			if ((currentEntryPrevOffset != 0)
				...
				threadSafeBufferPosition(readBuffer,
						(int) (currentEntryPrevOffset - readBufferFileStart));
			} else {
				 ...
				 * size, also start the next read buffer from the target LSN.
				 * Otherwise, try to position the next buffer chunk so the
				 * target entry is held within the buffer, all the way at the
				 * end.
				 */
				if (currentEntryPrevOffset == 0) {
					/* Go to another file. */
					currentEntryPrevOffset = fileManager
							.getFileHeaderPrevOffset(readBufferFileNum);
					Long prevFileNum = fileManager.getFollowingFileNum(
							readBufferFileNum, false);
					if (prevFileNum == null) {
						throw new EOFException();
					}
					if (readBufferFileNum - prevFileNum.longValue() != 1) {
						...
						if (!resyncReader(
								DbLsn.makeLsn(prevFileNum.longValue(),
										DbLsn.MAX_FILE_OFFSET), false)) {
				...
				} else if ((currentEntryOffset - currentEntryPrevOffset) > readBuffer
						.capacity()) {
				...
				/*
				 * Now that we've set readBufferFileNum and readBufferFileStart,
				 * do the read.
				 */
				FileHandle fileHandle = fileManager
						.getFileHandle(readBufferFileNum);
				try {
					readBuffer.clear();
			...
			if (finishLsn != DbLsn.NULL_LSN) {
				...
				long nextLsn = DbLsn
						.makeLsn(readBufferFileNum, nextEntryOffset);
	...
	private void readHeader(ByteBuffer dataBuffer) throws DatabaseException {
		 ...
		 */
		hook_checkType(currentEntryTypeNum);
	...
	private void hook_checkType(byte currentEntryTypeNum2)
			throws DatabaseException {
		if (!LogEntryType.isValidType(currentEntryTypeNum))
			throw new InvalidLogTypeException(env,
	 ...
	 * 
	 * @param amountToRead
	 *            is the number of bytes we need
	 * @param collectData
	 *            is true if we need to actually look at the data. If false, we
		...
		if (amountToRead > readBufferSize) {
			...
			if (readBufferSize < maxReadBufferSize) {
				...
				if (amountToRead < maxReadBufferSize) {
					...
					readBufferSize = Math
							.min(readBufferSize, maxReadBufferSize);
				} else {
			...
			if (amountToRead > readBuffer.capacity()) {
				hook_markRepeatIteratorRead();
			}
		}
	}

	private void hook_markRepeatIteratorRead() {
	}
	...
	private void copyToSaveBuffer(int bytesNeeded) {
		...
		saveBuffer.put(temp);
		threadSafeBufferPosition(readBuffer,
				threadSafeBufferPosition(readBuffer) + bytesFromThisBuffer);
	}

	/**
	 * Fill up the read buffer with more data.
	 */
	private void fillReadBuffer(int bytesNeeded) throws DatabaseException,
			EOFException {
		...
		try {
			...
			/*
			 * Check to see if we've come to the end of the file. If so, get the
			 * next file.
			 */
	...
	protected boolean isTargetEntry(byte logEntryTypeNumber,
			byte logEntryTypeVersion) throws DatabaseException {
	 ...
	 * 
	 * @param enteryBuffer
	 *            contains the entry data and is positioned at the data
