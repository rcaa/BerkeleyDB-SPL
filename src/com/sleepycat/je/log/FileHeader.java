 ...
 *
 * $Id: FileHeader.java,v 1.1.6.1.2.1 2006/10/10 17:19:45 ckaestne Exp $
 */
...
public class FileHeader implements LoggableObject, LogReadable {
	 ...
	 * See [12328]. Also added in Version 3 is IN LSN array compression. See
	 * [12557]. Also added in Version 3 is a change to FileSummaryLNs: obsolete
	 * offset tracking was added and multiple records are stored for a single
	 * file rather than a single record. Each record contains the offsets that
	 * were tracked since the last record was written. See [11597]. Also added
	 * in Version 3 is the full obsolete LSN in LNLogEntry.
	 */
	...
	boolean validate(String fileName, long expectedFileNum)
		...
		if (fileNum != expectedFileNum) {
			throw new LogException("Wrong filenum in header for file "
					+ fileName + " expected " + expectedFileNum + " got "
					+ fileNum);
		}
	...
	/**
	 * @see LoggableObject#writeToLog Serialize this object into the buffer.
	 *      Update cksum with all the bytes used by this object
	 * @param logBuffer
...
//FEATURE the ability to read older versioned logs
	/**
	 * @see LogReadable#readFromLog Initialize this object from the data in
	 *      itemBuf.
	 * @param itemBuf
	 *            the source buffer
	 */
