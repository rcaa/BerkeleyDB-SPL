 ...
 *
 * $Id: StatsFileReader.java,v 1.1.6.1 2006/07/28 09:01:54 ckaestne Exp $
 */
...
public class StatsFileReader extends DumpFileReader {
	 ...
	 */
	public StatsFileReader(EnvironmentImpl env, int readBufferSize,
			long startLsn, long finishLsn, String entryTypes, String txnIds,
			boolean verbose) throws IOException, DatabaseException {

		super(env, readBufferSize, startLsn, finishLsn, entryTypes, txnIds,
				verbose);
	...
	protected boolean processEntry(ByteBuffer entryBuffer)
		 ...
		 */
		LogEntryType lastEntryType = LogEntryType.findType(currentEntryTypeNum,
	...
	public void summarize() {
		...
		System.out.println(pad("type") + pad("total") + pad("provisional")
				+ pad("total") + pad("min") + pad("max") + pad("avg")
				+ pad("entries"));

		System.out.println(pad("") + pad("count") + pad("count") + pad("bytes")
				+ pad("bytes") + pad("bytes") + pad("bytes")
				+ pad("as % of log"));
		...
		while (iter.hasNext()) {
			...
			if (entryType == LogEntryType.LOG_LN_TRANSACTIONAL) {
				/*
				 * LN_TX entry overhead: 8 bytes node id 1 byte boolean (whether
				 * data exists or is null) 4 bytes data size 4 bytes key size 4
				 * bytes database id 8 bytes abort LSN 1 byte abortKnownDeleted
				 * 8 bytes txn id 8 bytes lastlogged LSN (backpointer for txn)
				 */
	...
	private void summarizeCheckpointInfo() {
		...
		/*
		 * Print out checkpoint interval info. If the log looks like this:
		 * 
		 * start of log ckpt1 start ckpt1 end ckpt2 start ckpt2 end end of log
		 * 
		 * There are 3 ckpt intervals start of log->ckpt1 end ckpt1 end -> ckpt2
		 * end ckpt2 end -> end of log
		 */
		System.out.println(pad("lnTxn") + pad("ln") + pad("mapLNTxn")
				+ pad("mapLN") + pad("end-end") + // ckpt n-1 end -> ckpt n
													// end
				pad("end-start") + // ckpt n-1 end -> ckpt n start
				pad("start-end") + // ckpt n start -> ckpt n end
				pad("maxLNReplay") + pad("ckptEnd"));
		...
		while (iter.hasNext()) {
			...
			if (prevCounter == null) {
				endToEndDistance = DbLsn.getWithCleaningDistance(end,
						fileManager, firstLsnRead, logFileMax);
			} else {
				endToEndDistance = DbLsn.getWithCleaningDistance(end,
						fileManager, prevCounter.endCkptLsn, logFileMax);
			}
			...
			if (prevCounter == null) {
				endToStartDistance = DbLsn.getWithCleaningDistance(start,
						fileManager, firstLsnRead, logFileMax);
			} else {
				endToStartDistance = DbLsn.getWithCleaningDistance(start,
						fileManager, prevCounter.endCkptLsn, logFileMax);
			...
			if ((c.startCkptLsn != DbLsn.NULL_LSN)
				...
				startToEndDistance = DbLsn.getWithCleaningDistance(
						c.endCkptLsn, fileManager, c.startCkptLsn, logFileMax);
			}
			sb.append(pad(form.format(startToEndDistance)));

			/*
			 * The maximum number of LNs to replay includes the portion of LNs
			 * from checkpoint start to checkpoint end of the previous interval.
			 */
	...
	static class CheckpointCounter {
		...
		public void increment(FileReader reader, byte currentEntryTypeNum) {
			...
			} else if (currentEntryTypeNum == LogEntryType.LOG_LN_TRANSACTIONAL
					.getTypeNum()) {
			...
			} else if (currentEntryTypeNum == LogEntryType.LOG_MAPLN
					.getTypeNum()) {
				if (startCkptLsn == DbLsn.NULL_LSN) {
					preStartMapLNCount++;
				} else {
			...
			} else if (currentEntryTypeNum == LogEntryType.LOG_MAPLN_TRANSACTIONAL
					.getTypeNum()) {
				if (startCkptLsn == DbLsn.NULL_LSN) {
