 ...
 *
 * $Id: LNFileReader.java,v 1.1.6.1 2006/07/28 09:01:52 ckaestne Exp $
 */
...
public class LNFileReader extends FileReader {
	 ...
	 * 
	 * @param env
	 *            The relevant EnvironmentImpl
	 * @param readBufferSize
	 *            buffer size in bytes for reading in log
	 * @param startLsn
	 *            where to start in the log
	 * @param redo
	 *            If false, we're going to go forward from the start LSN to the
	 *            end of the log. If true, we're going backwards from the end of
	 *            the log to the start LSN.
	 * @param endOfFileLsn
	 *            the virtual LSN that marks the end of the log. (The one off
	 *            the end of the log). Only used if we're reading backwards.
	 *            Different from the startLsn because the startLsn tells us
	 *            where the beginning of the start entry is, but not the
	 *            length/end of the start entry. May be null if we're going
	 *            foward.
	 * @param finishLsn
	 *            the last LSN to read in the log. May be null if we want to
	 *            read to the end of the log.
	 */
	public LNFileReader(EnvironmentImpl env, int readBufferSize, long startLsn,
			boolean redo, long endOfFileLsn, long finishLsn, Long singleFileNum)
			throws IOException, DatabaseException {

		super(env, readBufferSize, redo, startLsn, singleFileNum, endOfFileLsn,
				finishLsn);
