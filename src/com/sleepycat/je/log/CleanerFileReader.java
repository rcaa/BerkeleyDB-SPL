 ...
 *
 * $Id: CleanerFileReader.java,v 1.1.6.1 2006/07/28 09:01:46 ckaestne Exp $
 */
...
public class CleanerFileReader extends FileReader {
	 ...
	 * @param startLsn
	 *            where to start in the log, or null for the beginning.
	 * @param fileNum
	 *            single file number.
	 */
	public CleanerFileReader(EnvironmentImpl env, int readBufferSize,
			long startLsn, Long fileNum) throws IOException, DatabaseException {
	...
	private void addTargetType(byte category, LogEntryType entryType)
		...
		targetEntryMap.put(entryType, new EntryInfo(entryType.getNewLogEntry(),
				category));
	}
	...
	protected void initStartingPosition(long endOfFileLsn, Long fileNum)
		...
		/*
		 * Start off at the startLsn. If that's null, start at the beginning of
		 * the log. If there are no log files, set eof.
		 */
