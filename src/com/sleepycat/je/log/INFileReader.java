 ...
 *
 * $Id: INFileReader.java,v 1.1.6.1 2006/07/28 09:01:47 ckaestne Exp $
 */
...
public class INFileReader extends FileReader {
	 ...
	 * are LNs and the latter are INs. But nodeTrackingEntry and inTrackingEntry
	 * can overlap with the others, and we only load one of them when they do
	 * overlap.
	 */
	 ...
	 * check all MapLNs for the maximum db id, and check all LNs for the maximum
	 * txn id
	 */
	 ...
	 */
	public INFileReader(EnvironmentImpl env, int readBufferSize, long startLsn,
			long finishLsn, boolean trackIds, boolean mapDbOnly,
			long partialCkptStart, Map fileSummaryLsns) throws IOException,
			DatabaseException {
		...
		if (trackIds) {
			...
			fsTrackingEntry = (LNLogEntry) LogEntryType.LOG_FILESUMMARYLN
					.getNewLogEntry();
			...
			dbIdTrackingMap.put(LogEntryType.LOG_MAPLN, LogEntryType.LOG_MAPLN
					.getNewLogEntry());
			txnIdTrackingMap.put(LogEntryType.LOG_LN_TRANSACTIONAL,
					LogEntryType.LOG_LN_TRANSACTIONAL.getNewLogEntry());
	...
	/**
	 * If we're tracking node, database and txn ids, we want to see all node log
	 * entries. If not, we only want to see IN entries.
	 * 
	...
	protected boolean isTargetEntry(byte entryTypeNum, byte entryTypeVersion)
		...
		if (trackIds) {
			 ...
			 */
			if (!isProvisional) {
				dbIdTrackingEntry = (LNLogEntry) dbIdTrackingMap
						.get(fromLogType);
				txnIdTrackingEntry = (LNLogEntry) txnIdTrackingMap
						.get(fromLogType);
			}
			...
			if (fromLogType.isNodeType()) {
				if (possibleTarget != null) {
					...
					nodeTrackingEntry = (NodeLogEntry) otherNodeTrackingMap
							.get(fromLogType);
					if (nodeTrackingEntry == null) {
						nodeTrackingEntry = (NodeLogEntry) fromLogType
								.getNewLogEntry();
						otherNodeTrackingMap
								.put(fromLogType, nodeTrackingEntry);
					}
				}
			 ...
			 */
			return (targetLogEntry != null) || (dbIdTrackingEntry != null)
					|| (txnIdTrackingEntry != null)
					|| (nodeTrackingEntry != null);
		} else {
		...
		if (trackIds) {
			...
			if (dbIdTrackingEntry != null) {
				...
				lnEntry.readEntry(entryBuffer, currentEntrySize,
						currentEntryTypeVersion, true /* full load */);
				entryLoaded = true;
			...
			if (fsTrackingEntry != null) {
				...
				FileSummaryLN fsln = (FileSummaryLN) fsTrackingEntry
						.getMainItem();
			...
			if (nodeTrackingEntry != null) {
				if (!entryLoaded) {
					nodeTrackingEntry.readEntry(entryBuffer, currentEntrySize,
							currentEntryTypeVersion, false /* partial load */);
					entryLoaded = true;
				}
	 ...
	 * utilization profile. If true is returned, it should not be counted again,
	...
	private boolean isObsoleteLsnAlreadyCounted(long oldLsn, long newLsn) {
		...
		long fileSummaryLsn = DbLsn.longToLsn((Long) fileSummaryLsns
				.get(fileNum));
		int cmpFsLsnToNewLsn = (fileSummaryLsn != DbLsn.NULL_LSN) ? DbLsn
				.compareTo(fileSummaryLsn, newLsn) : -1;
	 ...
	 * that was just read. For BINDelta entries, it's the LSN of the last full
	 * version.
	 */
