 ...
 *
 * $Id: Sequence.java,v 1.1.6.4.2.3 2006/10/23 21:07:27 ckaestne Exp $
 */
...
//TODO	private Logger logger;

	/*
	 * The cache holds the range of values [cacheValue, cacheLast], which is the
	 * same as [cacheValue, storedValue) at the time the record is written. At
	 * store time, cacheLast is set to one before (after) storedValue.
	 * 
	 ...
	 */
	Sequence(Database db, OperationContext cxt, DatabaseEntry key,
		...
		SequenceConfig useConfig = (config != null) ? config
...
//TODO		logger = db.getEnvironment().getEnvironmentImpl().getLogger();
		...
		try {
			locker = LockerFactory.getWritableLocker(db.getEnvironment(),cxt,  false);
		...
		/*
		 * cacheLast is initialized such that the cache will be considered empty
	 ...
	 * </p>
	 */
	public synchronized long get(OperationContext cxt, int delta)
		 ...
		 * always <= Integer.MAX_VALUE, so we don't have to worry about overflow
		 * here as long as we subtract the two long values first.
		 */
		if ((increment && delta > ((cacheLast - cacheValue) + 1))
			...
			try {
				...
				if (overflow) {
					throw new DatabaseException("Sequence overflow "
							+ storedValue);
				}

				/*
				 * Handle wrapping. The range size can be larger than a long can
				 * hold, so to avoid arithmetic overflow we use BigInteger
				...
				if (increment) {
					...
					availBig = BigInteger.valueOf(rangeMax).subtract(
							BigInteger.valueOf(storedValue));
				} else {
					/* Available amount: storedValue - rangeMin */
					availBig = BigInteger.valueOf(storedValue).subtract(
							BigInteger.valueOf(rangeMin));
				}
...
//TODO		/* Trace this method at the FINEST level. */
//		if (logger.isLoggable(Level.FINEST)) {
//			logger.log(Level.FINEST, "Sequence.get" + " value=" + retVal
...
//		}
	...
	private void hook_getDone(boolean cached) {
	...
	/**
	 * Reads persistent fields from the sequence record. Throws an exception if
	...
	private DatabaseEntry copyEntry(DatabaseEntry entry) {
		...
		if (len == 0) {
			...
			data = new byte[len];
			System.arraycopy(entry.getData(), entry.getOffset(), data, 0,
					data.length);
		}
