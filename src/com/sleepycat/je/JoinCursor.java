 ...
 *
 * $Id: JoinCursor.java,v 1.2.6.2 2006/07/28 14:07:17 ckaestne Exp $
 */
...
public class JoinCursor {
	 ...
	 */
	JoinCursor(Locker locker, Database primaryDb, final Cursor[] cursors,
			JoinConfig configParam) throws DatabaseException {
		...
		if (!config.getNoSort()) {
			...
			/*
			 * Sort ascending by duplicate count. Collect counts before sorting
			 * so that countInternal() is called only once per cursor. Use
			 * READ_UNCOMMITTED to avoid blocking writers.
			 */
		...
		/*
		 * Open and dup cursors last. If an error occurs before the constructor
		 * is complete, close them and ignore exceptions during close.
		 */
		try {
	 ...
	 *            an exception that has already occured, or null.
	 */
	...
	public OperationStatus getNext(DatabaseEntry key, LockMode lockMode)
		...
		// refined trace priCursor.trace(Level.FINEST, "JoinCursor.getNext(key):
		// ", lockMode);
	...
	public OperationStatus getNext(DatabaseEntry key, DatabaseEntry data,
		...
		// refined trace priCursor.trace(Level.FINEST,
	 ...
	 * looking for each candidate key in the duplicate set of the other cursors,
	 * without ever reseting a cursor to the beginning of the duplicate set.
	 * <p>
	 * This only works when the same duplicate comparison method is used for all
	 * cursors. We don't check for that, we just assume the user won't violate
	 * that rule.
	 * <p>
	 * A future optimization would be to add a SearchMode.BOTH_DUPS operation
	 * and use it instead of using SearchMode.BOTH. This would be the equivalent
	 * of the undocumented DB_GET_BOTHC operation used by DB core's join()
	 * implementation.
	 */
	private OperationStatus retrieveNext(DatabaseEntry keyParam,
			DatabaseEntry dataParam, LockMode lockMode)
			throws DatabaseException {
		...
		outerLoop: while (true) {
			...
			if (dataParam != null) {
				status = priCursor.search(candidateKey, dataParam, lockMode,
						SearchMode.SET);
				if (status != OperationStatus.SUCCESS) {
