 ...
 *
 * $Id: Locker.java,v 1.1.6.4.2.3 2006/10/22 01:08:25 ckaestne Exp $
 */
 ...
 * AutoTxn. Locker instances are in fact only a transaction shell to get to the
 * lock manager, and don't guarantee transactional semantics. Txn and AutoTxn
 * instances are both truely transactional, but have different ending behaviors.
 */
public abstract class Locker {
	 ...
	 * database handles. This is maintained as a map where the key is the handle
	 * lock id and the value is a set of database handles that correspond to
	 * that handle lock. This is a 1 - many relationship because a single handle
	 * lock can cover multiple database handles opened by the same transaction.
	 */
	 ...
	 * 
	 * @param lockManager
	 *            lock manager for this environment
	 * @param readUncommittedDefault
	 *            if true, this transaction does read-uncommitted by default
	 * @param noWait
	 *            if true, non-blocking lock requests are used.
	 */
	public Locker(EnvironmentImpl envImpl, boolean readUncommittedDefault,
			boolean noWait) throws DatabaseException {
	...
	/**
	 * Set the lock timeout period for any locks in this transaction, in
	 * milliseconds.
	 */
	 ...
	 * 
	 * @param lockType
	 *            is the type of lock to request.
	 * 
	 * @param noWait
	 *            is true to override the defaultNoWait setting. If true, or if
	 *            defaultNoWait is true, throws LockNotGrantedException if the
	 ...
	 *             if acquiring a blocking lock would result in a deadlock.
	 */
	/*ChK_ made public for Transaction Feature*/
	public abstract LockResult lockInternal(long nodeId, LockType lockType,
			boolean noWait, DatabaseImpl database) throws DeadlockException,
			DatabaseException;
	 ...
	 *             if acquiring a blocking lock would result in a deadlock.
	 */
	public LockResult lock(long nodeId, LockType lockType, boolean noWait,
			DatabaseImpl database) throws LockNotGrantedException,
			DeadlockException, DatabaseException {
	 ...
	 * 
	 * <p>
	 * Unlike lock(), this method returns LockGrantType.DENIED if the lock is
	 * denied rather than throwing LockNotGrantedException. This method should
	 * therefore not be used as the final lock for a user operation, since in
	 * that case LockNotGrantedException should be thrown for a denied lock. It
	 * is normally used only to probe for a lock, and other recourse is taken if
	 * the lock is denied.
	 * </p>
	 * 
	 * @param nodeId
	 ...
	 *            is the database containing nodeId.
	 */
	public LockResult nonBlockingLock(long nodeId, LockType lockType,
	...
	public void releaseLock(long nodeId) throws DatabaseException {
		...
		/*
		 * If successful, the lock manager will call back to the transaction and
		 * remove the lock from the lock collection. Done this way because we
	...
	public void demoteLock(long nodeId) throws DatabaseException {
		...
		/*
		 * If successful, the lock manager will call back to the transaction and
		 * adjust the location of the lock in the lock collection.
		 */
	...
	/**
	 * Returns the underlying Txn if the locker is transactional, or null if the
	 * locker is non-transactional. For a Txn-based locker, this method returns
	 * 'this'. For a BuddyLocker, this method may returns the buddy.
	 */
	...
	/**
	 * Releases any non-transactional locks held by this locker. This method is
	 * called when the cursor moves to a new position or is closed.
	 * 
	 * <p>
	 * In general, transactional lockers do nothing when this method is called,
	 * while non-transactional lockers release all locks as if operationEnd were
	 * called.
	 * </p>
	 */
	 ...
	 * 
	 * <p>
	 * All lockers share locks with a BuddyLocker whose buddy is this locker. To
	 * support BuddyLocker when overriding this method, always return true if
	 * this implementation (super.sharesLocksWith(...)) returns true.
	 * </p>
	 */
	 ...
	 * 
	 * @param operationOK
	 *            is whether the operation succeeded, since that may impact
	 *            ending behavior. (i.e for AutoTxn)
	 */
	 ...
	 */
	abstract void addLock(Long nodeId, Lock lock, LockType type,
			LockGrantType grantStatus) throws DatabaseException;

	/**
	 * @return true if this transaction created this node, for a operation with
	 ...
	 * 
	 */
	public void transferHandleLock(Database dbHandle, Locker destLocker,
	...
	public String toString() {
		...
		return Long.toString(id) + "_"
				+ ((thread == null) ? "" : thread.getName()) + "_" + className;
	}
