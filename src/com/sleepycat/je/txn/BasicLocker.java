 ...
 *
 * $Id: BasicLocker.java,v 1.1.6.3.2.2 2006/10/22 01:08:26 ckaestne Exp $
 */
...
public class BasicLocker extends Locker {
	 ...
	 * instantiated and used only if more than one lock is obtained. This is an
	 * optimization for the common case where only one lock is held by a
	...
	public void operationEnd(boolean operationOK) throws DatabaseException {
		...
		synchronized (this) {
			if ((deleteInfo != null) && (deleteInfo.size() > 0)) {
				envImpl.addToCompressorQueue(deleteInfo.values(), false); // no
																			// wakeup
				deleteInfo.clear();
			}
		}
