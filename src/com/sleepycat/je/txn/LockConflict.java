 ...
 *
 * $Id: LockConflict.java,v 1.1.6.1 2006/07/28 09:01:34 ckaestne Exp $
 */
...
class LockConflict {
	...
	/**
	 * This method is called first to determine whether the locks is allowed. If
	 * true, there is no conflict. If false, there is a conflict and the
	 * requester must wait for or be denied the lock, or (if getRestart returns
	 * true) an exception should be thrown to cause the requester's operation to
	 * be restarted.
	 */
	boolean getAllowed() {
		return allowed;
	 ...
	 * restarted. If getAllowed returns false and this method returns false, the
	 * requester should wait for or be denied the lock, depending on the request
