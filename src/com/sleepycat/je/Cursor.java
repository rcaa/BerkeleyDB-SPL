 ...
 *
 * $Id: Cursor.java,v 1.2.6.4.2.4 2006/10/23 21:07:26 ckaestne Exp $
 */
...
public class Cursor {
	 ...
	 * and
	 * 
	 * (2) The user did not supply a txn to the cursor ctor (meaning, the locker
	 ...
	 * 
	 * <p>
	 * If txn is null, a non-transactional cursor will be created that releases
	 * locks for the prior operation when the next operation suceeds.
	 * </p>
	 */
	Cursor(Database dbHandle, OperationContext cxt, CursorConfig cursorConfig)
		...
		Locker locker = LockerFactory.getReadableLocker(dbHandle
				.getEnvironment(),cxt, false /* retainNonTxnLocks */, false
		);

		init(dbHandle, dbHandle.getDatabaseImpl(), locker, dbHandle
				.isWritable(), cursorConfig);
	}
	 ...
	 * will be created that releases locks for the prior operation when the next
	 * operation suceeds.
	 * </p>
	 */
	Cursor(Database dbHandle, Locker locker, CursorConfig cursorConfig)
		...
		// ChK_ need hooker for modified cursorConfig
		locker = hook_getReadableLocker(dbHandle, locker, cursorConfig);

		init(dbHandle, dbHandle.getDatabaseImpl(), locker, dbHandle
				.isWritable(), cursorConfig);
	}

	private Locker hook_getReadableLocker(Database dbHandle, Locker locker,
			CursorConfig cursorConfig) throws DatabaseException {
		return LockerFactory.getReadableLocker(dbHandle.getEnvironment(),dbHandle,
				locker, false // retainNonTxnLocks
				, false);
	}
	 ...
	 * 
	 * <p>
	 * The locker parameter must be non-null. With this constructor, we use the
	 * given locker without applying any special rules for different isolation
	 * levels -- the caller must supply the correct locker.
	 * </p>
	 */
	...
	private void init(Database dbHandle, DatabaseImpl dbImpl, Locker locker,
			boolean isWritable, CursorConfig cursorConfig)
			throws DatabaseException {

		assert locker != null;
		assert dbImpl != null;

		cursorImpl = new CursorImpl(dbImpl, locker, false /* retainNonTxnLocks */);

		readUncommittedDefault = cursorConfig.getReadUncommitted()
				|| locker.isReadUncommittedDefault();

		serializableIsolationDefault = cursorImpl.getLocker()
				.isSerializableIsolation();

		updateOperationsProhibited = hook_getUpdateOperationsProhibited(isWritable);
	...
	private boolean hook_getUpdateOperationsProhibited(boolean isWritable) {
		return !isWritable;
	}
	...
	/**
	 * Always returns non-null, while getDatabase() returns null if no handle is
	...
	public int count() throws DatabaseException {
		...
		/*
		 * Specify a null LockMode to use default locking. The API doesn't allow
		 * specifying a lock mode, but we should at least honor the configured
		 * default.
		 */
	...
	public OperationStatus getSearchKeyRange(DatabaseEntry key,
		...
		DatabaseUtil.checkForNullDbt(data, "data", false);
		// refined trace:
		// trace(Level.FINEST, "Cursor.getSearchKeyRange: ", key, null,
		// lockMode);
	...
	OperationStatus deleteInternal() throws DatabaseException {
		 ...
		 */
		if (doNotifyTriggers) {
			dbHandle.notifyTriggers(cursorImpl.getLocker(), oldKey, oldData,
					null);
		}
	...
	OperationStatus deleteNoNotify() throws DatabaseException {
		...
		try {
			...
			hook_afterDeletion(original, dup);
	...
	private void hook_afterDeletion(CursorImpl original, CursorImpl dup) throws DatabaseException {
	}
	...
	OperationStatus putInternal(DatabaseEntry key, DatabaseEntry data,
			PutMode putMode) throws DatabaseException {

		/* Need to get existing data if updating secondaries. */
		DatabaseEntry oldData = null;
		boolean doNotifyTriggers = dbHandle != null && dbHandle.hasTriggers();
		if (doNotifyTriggers
				&& (putMode == PutMode.CURRENT || putMode == PutMode.OVERWRITE)) {
			oldData = new DatabaseEntry();
	...
	/**
	 * Performs the put operation but does not notify triggers (does not perform
	 * secondary updates). Prevents phantoms.
	 */
	OperationStatus putNoNotify(DatabaseEntry key, DatabaseEntry data,
			PutMode putMode, DatabaseEntry returnOldData)
			throws DatabaseException {
		...
		try {
			...
			if (putMode != PutMode.CURRENT
					&& dbImpl.getDbEnvironment().getTxnManager()
							.areOtherSerializableTransactionsActive(
									cursorLocker)) {
				nextKeyLocker = new BuddyLocker(dbImpl.getDbEnvironment(),
						cursorLocker);
			...
			return putAllowPhantoms(key, data, putMode, returnOldData,
					nextKeyCursor);
		} finally {
	 ...
	 * 
	 * @param nextKeyCursor
	 *            is the cursor used to lock the next key during phantom
	 *            prevention. If this cursor is non-null and initialized, it's
	 *            BIN will be used to initialize the dup cursor used to perform
	 *            insertion. This enables an optimization that skips the search
	 *            for the BIN.
	 */
	private OperationStatus putAllowPhantoms(DatabaseEntry key,
			DatabaseEntry data, PutMode putMode, DatabaseEntry returnOldData,
			CursorImpl nextKeyCursor) throws DatabaseException {
	...
	private void hook_afterPut(CursorImpl original) throws DatabaseException {
	}

	/**
	 * Position the cursor at the first or last record of the database. Prevents
	 * phantoms.
	 */
	OperationStatus position(DatabaseEntry key, DatabaseEntry data,
			LockMode lockMode, boolean first) throws DatabaseException {

		if (!isSerializableIsolation(lockMode)) {
			return positionAllowPhantoms(key, data,
					getLockType(lockMode, false), first);
		}
		...
		while (true) {
			try {
				...
				/* Perform operation. */
				OperationStatus status = positionAllowPhantoms(key, data,
						lockType, first);
	...
	private OperationStatus positionAllowPhantoms(DatabaseEntry key,
			DatabaseEntry data, LockType lockType, boolean first)
			throws DatabaseException {
		...
		try {
			...
			if (!dup.positionFirstOrLast(first, null)) {
				...
				hook_assertOneLatch();
				status = dup.getCurrent(key, data, lockType, first);
				...
				if (status == OperationStatus.SUCCESS) {
					...
					status = dup
							.getNext(key, data, lockType, first/* Lck, false */);
				}
			}
		} finally {

			/*
			 * positionFirstOrLast returns with the target BIN latched, so it is
	...
	private void hook_assertOneLatch() {
	}
	...
	OperationStatus search(DatabaseEntry key, DatabaseEntry data,
			LockMode lockMode, SearchMode searchMode) throws DatabaseException {

		if (!isSerializableIsolation(lockMode)) {
			LockType lockType = getLockType(lockMode, false);
			KeyChangeStatus result = searchAllowPhantoms(key, data, lockType,
					lockType, searchMode);
		...
		while (true) {
			try {
				...
				DatabaseEntry tryKey = new DatabaseEntry(key.getData(), key
						.getOffset(), key.getSize());
				DatabaseEntry tryData = new DatabaseEntry(data.getData(), data
				...
				if (searchMode.isExactSearch()) {
					...
					result = searchExactAndRangeLock(tryKey, tryData,
							searchLockType, advanceLockType, searchMode);
				} else {
					/* Normal range search. */
					result = searchAllowPhantoms(tryKey, tryData,
							searchLockType, advanceLockType, searchMode);
	 ...
	 * key changes (or if the data changes for BOTH) during the search. If no
	 * exact match is found the range search will range lock the following key
	 * for phantom prevention. Importantly, the cursor position is not changed
	 * if an exact match is not found, even though we advance to the following
	 * key in order to range lock it.
	 */
	private KeyChangeStatus searchExactAndRangeLock(DatabaseEntry key,
			DatabaseEntry data, LockType searchLockType,
			LockType advanceLockType, SearchMode searchMode)
			throws DatabaseException {

		/* Convert exact search to range search. */
		searchMode = (searchMode == SearchMode.SET) ? SearchMode.SET_RANGE
				: SearchMode.BOTH_RANGE;

		KeyChangeStatus result = null;
		boolean noNextKeyFound;

		CursorImpl dup = beginRead(false /* searchAndPosition will add cursor */);
		...
		try {
			 ...
			 */
			result = searchInternal(dup, key, data, searchLockType,
					advanceLockType, searchMode, true /* advanceAfterRangeSearch */);
	...
	private KeyChangeStatus searchAllowPhantoms(DatabaseEntry key,
		...
		try {
			KeyChangeStatus result = searchInternal(dup, key, data,
					searchLockType, advanceLockType, searchMode, false /* advanceAfterRangeSearch */);
	 ...
	 */
	private KeyChangeStatus searchInternal(CursorImpl dup, DatabaseEntry key,
			DatabaseEntry data, LockType searchLockType,
			LockType advanceLockType, SearchMode searchMode,
			boolean advanceAfterRangeSearch) throws DatabaseException {
		...
		boolean keyChange = false;
			...
			/* search */
			int searchResult = dup.searchAndPosition(key, data, searchMode,
					searchLockType);
			if ((searchResult & CursorImpl.FOUND) != 0) {

				/*
				 * The search found a possibly valid record.
				 * CursorImpl.searchAndPosition's job is to settle the cursor at
				 * a particular location on a BIN. In some cases, the current
				 * position may not actually hold a valid record, so it's this
				 * layer's responsiblity to judge if it might need to bump the
				 * cursor along and search more. For example, we might have to
				 * do so if the position holds a deleted record.
				 * 
				 ...
				 * CursorImpl.searchAndPosition gave us an exact answer. However
				 * since it doesn't peer into the duplicate set, we may need to
				 * probe further in if there are deleted records in the
				 * duplicate set. i.e, we have to be able to find k1/d2 even if
				 * there's k1/d1(deleted), k1/d2, k1/d3, etc in a duplicate set.
				 * 
				 ...
				 * complete match. If SET_RANGE was spec'd and EXACT_KEY was not
				 * returned as set, then the key didn't match exactly. If
				 * BOTH_RANGE was spec'd and EXACT_DATA was not returned as set,
				 * then the data didn't match exactly.
				 */
				 ...
				 */
				DatabaseEntry useKey = (searchMode == SearchMode.SET) ? null
						: key;
				 ...
				 * have an exact match) but the entry is deleted. So we test for
				 * rangeMatch or a deleted entry, and if either is true then we
				 * advance to the next non-deleted entry.
				 */
				if (rangeMatch
						|| (status = dup.getCurrent(useKey, data,
								searchLockType, true)) == OperationStatus.KEYEMPTY) {
					...
					if (foundLast) {
						 ...
						 * rangeMatch, it's a deleted record. We should advance,
						 * but only to duplicates for the same key.
						 */
						status = dup.getNextDuplicate(key, data,
								advanceLockType, true/* , Lck rangeMatch */);
						 ...
						 */
						if (exactKeyMatch) {
							KeyChangeStatus result = dup
									.getNextWithKeyChangeStatus(key, data,
											advanceLockType, true/*
																	 * Lck ,
																	 * rangeMatch
																	 */);
							status = result.status;
							 ...
							 */
							keyChange = searchMode.isDataSearch() ? (status == OperationStatus.SUCCESS)
									: result.keyChange;
							 ...
							 */
							status = dup.getNextNoDup(key, data,
									advanceLockType, true/* Lck, rangeMatch */);
						...
						if (status == OperationStatus.SUCCESS
							...
							if (Key.compareKeys(key.getData(), searchKey,
	...
	OperationStatus retrieveNext(DatabaseEntry key, DatabaseEntry data,
			LockMode lockMode, GetMode getMode) throws DatabaseException {

		if (!isSerializableIsolation(lockMode)) {
			return retrieveNextAllowPhantoms(key, data, getLockType(lockMode,
					false), getMode);
		}
		...
		while (true) {
			try {
				...
				if (getMode == GetMode.NEXT_DUP) {
					...
					LockType lockType = getLockType(lockMode, getMode
							.isForward());

					/* Perform the operation. */
					status = retrieveNextAllowPhantoms(key, data, lockType,
							getMode);
	 ...
	 * following key for phantom prevention. Importantly, the cursor position is
	 * not changed if there are no more dups, even though we advance to the
	...
	private OperationStatus getNextDupAndRangeLock(DatabaseEntry key,
		...
		/*
		 * Perform a NEXT and return NOTFOUND if the key changes during the
		 * search.
		 */
		while (true) {
			hook_assertNoLatches();
			...
			try {
				KeyChangeStatus result = dup.getNextWithKeyChangeStatus(tryKey,
						tryData, lockType, true/* Lck, false */);
				status = result.status;
	 ...
	 * if not found, on the logical EOF node. Do not modify the current cursor
	 * position, use a separate cursor.
	 */
	private void rangeLockCurrentPosition(GetMode getMode)
		...
		try {
			if (getMode == GetMode.PREV_NODUP) {
				status = dup.getFirstDuplicate(tempKey, tempData,
						LockType.RANGE_READ);
			} else {
			...
			if (status != OperationStatus.SUCCESS) {
				while (true) {
					...
					status = dup.getNext(tempKey, tempData,
							LockType.RANGE_READ, true/* Lck, false */);
	...
	private OperationStatus retrieveNextAllowPhantoms(DatabaseEntry key,
			DatabaseEntry data, LockType lockType, GetMode getMode)
			throws DatabaseException {
		...
		while (true) {
			...
			try {
				if (getMode == GetMode.NEXT) {
					status = dup
							.getNext(key, data, lockType, true/* Lck, false */);
				} else if (getMode == GetMode.PREV) {
					status = dup
							.getNext(key, data, lockType, false/* Lck, false */);
				} else if (getMode == GetMode.NEXT_DUP) {
					status = dup.getNextDuplicate(key, data, lockType, true/*
																			 * Lck,
																			 * false
																			 */);
				} else if (getMode == GetMode.PREV_DUP) {
					status = dup.getNextDuplicate(key, data, lockType, false/*
																			 * Lck,
																			 * false
																			 */);
				} else if (getMode == GetMode.NEXT_NODUP) {
					status = dup
							.getNextNoDup(key, data, lockType, true/*
																	 * Lck,
																	 * false
																	 */);
				} else if (getMode == GetMode.PREV_NODUP) {
					status = dup
							.getNextNoDup(key, data, lockType, false/*
																	 * Lck,
																	 * false
																	 */);
				} else {
	 ...
	 */
	OperationStatus getCurrentInternal(DatabaseEntry key, DatabaseEntry data,
			LockMode lockMode) throws DatabaseException {
	 ...
	 * 
	 * Note that when we call LN.isDeleted(), we do not need to lock the LN. If
	 * we see a non-committed deleted entry, we'll just iterate around in the
	 * caller. So a false positive is ok.
	 * 
	 * @return true if an unaccounted for insertion happened.
	 */
	private boolean checkForInsertion(GetMode getMode, CursorImpl origCursor,
			CursorImpl dupCursor) throws DatabaseException {
		...
		if (getMode == GetMode.PREV || getMode == GetMode.PREV_DUP
		...
		if (origBIN != dupBIN) {
			...
			ret=hook_checkInternal(origCursor, origDBIN, forward, origBIN);
			return ret;
		}
		...
		if (origDBIN != dupCursor.getDupBIN()
			...
			ret=hook_checkInternal2(origCursor,  origDBIN, forward, origBIN);
			return ret;
		}
		return false;
	}

	private boolean hook_checkInternal2(CursorImpl origCursor, DBIN origDBIN, boolean forward, BIN origBIN) throws DatabaseException {
		boolean ret=false;
		if (forward) {
			if (origDBIN.getNEntries() - 1 > origCursor.getDupIndex()) {
				 ...
				 */
				for (int i = origCursor.getDupIndex() + 1; i < origDBIN
						.getNEntries(); i++) {
	...
	private boolean hook_checkInternal(CursorImpl origCursor, DBIN origDBIN, boolean forward, BIN origBIN) throws DatabaseException {
		boolean ret=false;
		if (origDBIN == null) {
			if (forward) {
				if (origBIN.getNEntries() - 1 > origCursor.getIndex()) {
					 ...
					 */
					for (int i = origCursor.getIndex() + 1; i < origBIN
							.getNEntries(); i++) {
	 ...
	 * return the original. This avoids the overhead of duping when the original
	 * is uninitialized. The cursor returned must be passed to endRead() to
	...
	private LockType getLockType(LockMode lockMode, boolean rangeLock) {
		...
		if (isReadUncommittedMode(lockMode)) {
			...
			throw new IllegalArgumentException(lockMode.toString()
	...
	// */
	// void trace(Level level, String methodName, DatabaseEntry key,
