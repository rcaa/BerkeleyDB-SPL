 ...
 *
 * $Id: CursorImpl.java,v 1.1.6.2.2.9 2006/10/26 16:36:49 ckaestne Exp $
 */
...
public class CursorImpl implements Cloneable {
	void hook_walker() throws DatabaseException {
	...
	/*
	 * Cursor location in the database, represented by a BIN and an index in the
	 * BIN. bin/index must have a non-null/non-negative value if dupBin is set
	 * to non-null.
	 */
	...
	/*
	 * Allocate a new hashCode id. Doesn't need to be synchronized since it's ok
	 * for two objects to have the same hashcode.
	 */
	 ...
	 *            is true if non-transactional locks should be retained (not
	 *            released automatically) when the cursor is closed.
	 */
	public CursorImpl(DatabaseImpl database, Locker locker,
			boolean retainNonTxnLocks) throws DatabaseException {
		 ...
		 * Database.openCursor(). Also eviction may be disabled after the cursor
		 * is constructed.
		 */
	}
	...
	public CursorImpl cloneCursor(boolean addCursor, CursorImpl usePosition)
		...
		if (nonCloning) {
			...
			ret = hook_cloneCursor(addCursor, usePosition);
		}
	...
	private CursorImpl hook_cloneCursor(boolean addCursor,
			CursorImpl usePosition) throws DatabaseException {
		CursorImpl ret = null;
		try {
	 ...
	 * 
	 * @param key
	 *            on return contains the key if available, or null.
	 * @param data
	 *            on return contains the data if available, or null.
	 */
	public boolean advanceCursor(DatabaseEntry key, DatabaseEntry data) {
		...
		try {
			getNext(key, data, LockType.NONE, true /* forward *//*
																 * Lck false /*
																 * alreadyLatched
																 */);
		 ...
		 */
		if (bin != oldBin || dupBin != oldDupBin || index != oldIndex
				|| dupIndex != oldDupIndex) {

			/*
			 * Return the key and data from the BIN entries, if we were not able
			 * to read it above.
			 */
	...
	/*
	 * Update a cursor to refer to a new BIN or DBin following an insert. Don't
	 * bother removing this cursor from the previous bin. Cursor will do that
	 * with a cursor swap thereby preventing latch deadlocks down here.
	 */
	 ...
	 */
	public void clearDupBIN() throws DatabaseException {
	 ...
	 * 
	 * @throws DatabaseException
	 *             if the cursor was previously closed.
	 */
	...
	public int count(LockType lockType) throws DatabaseException {
		...
		return hook_count(lockType);
	}

	private int hook_count(LockType lockType) throws DatabaseException {
		...
		if (n != null && n.containsDuplicates()) {
			...
			DupCountLN dupCountLN = hook_fetchDuplicate(dupRoot);

			/*
			 * Call lock directly. There is no need to call lockLN because the
			 * node ID cannot change (a slot cannot be reused) for a DupCountLN.
			 */
			if (lockType != LockType.NONE) {
				locker.lock(dupCountLN.getNodeId(), lockType,
						false /* noWait */, database);
			}
	...
	private DupCountLN hook_fetchDuplicate(DIN dupRoot)
			throws DatabaseException {
		return (DupCountLN) dupRoot.getDupCountLNRef().fetchTarget(database,
				dupRoot);
	}
	...
	public OperationStatus delete() throws DatabaseException {
		...
		try {
			 ...
			 * any number of operations may have executed which would result in
			 * a new abort LSN for this record. Therefore, wait until now to get
			 * the abort LSN.
			 */
			setTargetBin();
			long oldLsn = targetBin.getLsn(targetIndex);
			byte[] lnKey = targetBin.getKey(targetIndex);
			lockResult.setAbortLsn(oldLsn, targetBin
					.isEntryKnownDeleted(targetIndex));

			/* Log the LN. */
			long newLsn = hook_lnDelete(targetBin, ln, oldLsn, lnKey);

			/*
			 * Now update the parent of the LN (be it BIN or DBIN) to correctly
			 * reference the LN and adjust the memory sizing. Be sure to do this
			 * update of the LSN before updating the dup count LN. In case we
			 * encounter problems there we need the LSN to match the latest
			...
			if (isDup) {
				dupRoot.incrementDuplicateCount(dclLockResult, dupKey, locker,
						false /* increment */);
				hook_releaseLatch(dupRoot);
				dupRoot = null;
			...
			// TODO trace(Level.FINER, TRACE_DELETE, targetBin, ln, targetIndex,
			// oldLsn, newLsn);
		} finally {
			if (dupRoot != null) {
				hook_releaseLatchIfOwner(dupRoot);
			}
		}
	...
	private long hook_lnDelete(BIN targetBin, LN ln, long oldLsn, byte[] lnKey) throws DatabaseException {
		long newLsn = ln.delete(database, lnKey, dupKey, oldLsn, locker);
		return newLsn;
	}

	void hook_releaseBIN() throws DatabaseException {
	}

	void hook_releaseBINs() throws DatabaseException {
	}

	private void hook_releaseLatchIfOwner(IN dupRoot) throws DatabaseException {
	}

	private void hook_releaseLatch(IN dupRoot) throws DatabaseException {
	}

	private void hook_latch(IN dupRoot) throws DatabaseException {
	}
	...
	public void lockNextKeyForInsert(DatabaseEntry key, DatabaseEntry data)
		...
		DatabaseEntry tempKey = new DatabaseEntry(key.getData(), key
				.getOffset(), key.getSize());
		DatabaseEntry tempData = new DatabaseEntry(data.getData(), data
				.getOffset(), data.getSize());
		...
		SearchMode searchMode = database.getSortedDuplicates() ? SearchMode.BOTH_RANGE
				: SearchMode.SET_RANGE;

		/* Search. */
		hook_lookNextKeyForInsertInner(tempKey, tempData, searchMode,
				lockedNextKey);
	...
	private boolean hook_lookNextKeyForInsertInner(DatabaseEntry tempKey,
			DatabaseEntry tempData, SearchMode searchMode, boolean lockedNextKey)
			throws DatabaseException {
		int searchResult = searchAndPosition(tempKey, tempData, searchMode,
				LockType.RANGE_INSERT);
		if ((searchResult & FOUND) != 0 && (searchResult & FOUND_LAST) == 0) {

			/*
			 * If searchAndPosition found a record (other than the last one), in
			 * all cases we should advance to the next record:
			 * 
			 * 1- found a deleted record, 2- found an exact match, or 3- found
			 * the record prior to the given key/data.
			 * 
			 * If we didn't match the key, skip over duplicates to the next key
			 * with getNextNoDup.
			 */
			OperationStatus status;
			if ((searchResult & EXACT_KEY) != 0) {
				status = getNext(tempKey, tempData, LockType.RANGE_INSERT, true/*
																				 * Lck,
																				 * true
																				 */);
			} else {
				status = getNextNoDup(tempKey, tempData, LockType.RANGE_INSERT,
						true/* Lck, true */);
			}
	...
	public OperationStatus putLN(byte[] key, LN ln, boolean allowDuplicates)
		...
		LockResult lockResult = locker.lock(ln.getNodeId(), LockType.WRITE,
				false /* noWait */, database);
		 ...
		 */
		if (database.getTree().insert(ln, key, allowDuplicates, this,
				lockResult)) {
	...
	public OperationStatus put(DatabaseEntry key, DatabaseEntry data,
			DatabaseEntry foundData) throws DatabaseException {

		assert assertCursorState(false) : dumpToString(true);

		OperationStatus result = putLN(Key.makeKey(key), new LN(data), database
				.getSortedDuplicates());
	...
	public OperationStatus putCurrent(DatabaseEntry data,
		...
		return hook_putCurrentInternal(data, foundKey, foundData);

	}

	private OperationStatus hook_putCurrentInternal(DatabaseEntry data,
			DatabaseEntry foundKey, DatabaseEntry foundData)
			throws DatabaseException {
		boolean isDup = setTargetBin();
		/*
		 * Find the existing entry and get a reference to all BIN fields while
		 * latched.
		 */
		LN ln = (LN) targetBin.fetchTarget(targetIndex);
		byte[] lnKey = targetBin.getKey(targetIndex);
		...
		/*
		 * If cursor points at a dup, then we can only replace the entry with a
		 * new entry that is "equal" to the old one. Since a user defined
		 * comparison function may actually compare equal for two byte sequences
		 * that are actually different we still have to do the replace. Arguably
		 * we could skip the replacement if there is no user defined comparison
		 * function and the new data is the same.
		 */
		byte[] foundDataBytes;
		byte[] foundKeyBytes;
		isDup = setTargetBin();
		...
		if (data.getPartial()) {
			...
			System.arraycopy(data.getData(), data.getOffset(), newData, pos,
					slicelen);
			pos += slicelen;
		...
		if (database.getSortedDuplicates()) {
			...
			if (foundDataBytes != null) {
				keysEqual = Key.compareKeys(foundDataBytes, newData,
						userComparisonFcn) == 0;
			}
			if (!keysEqual) {
		...
		/*
		 * Between the release of the BIN latch and acquiring the write lock any
		 * number of operations may have executed which would result in a new
		 * abort LSN for this record. Therefore, wait until now to get the abort
		 * LSN.
		 */
		long oldLsn = targetBin.getLsn(targetIndex);
		lockResult.setAbortLsn(oldLsn, targetBin
		...
		long newLsn = hook_lnModify(targetBin,ln, newData, oldLsn, newKey);

		/* Update the parent BIN. */
		targetBin.updateEntry(targetIndex, newLsn);

		// TODO trace(Level.FINER, TRACE_MOD, targetBin, ln, targetIndex,
		// oldLsn,
		// newLsn);
	...
	private long hook_lnModify(BIN targetBin2, LN ln, byte[] newData, long oldLsn, byte[] newKey) throws DatabaseException {
	...
	public OperationStatus getCurrent(DatabaseEntry foundKey,
			DatabaseEntry foundData, LockType lockType)
			throws DatabaseException {

		return getCurrent(foundKey, foundData, lockType, true);
	}

	public OperationStatus getCurrent(DatabaseEntry foundKey,
			DatabaseEntry foundData, LockType lockType, boolean first)
			throws DatabaseException {
	...
	public LN getCurrentLN(LockType lockType) throws DatabaseException {
		...
		if (bin == null) {
			...
			return hook_getCurrentLNInternal(lockType);
		}
	}

	private LN hook_getCurrentLNInternal(LockType lockType)
			throws DatabaseException {
		assert assertCursorState(true) : dumpToString(true);
		...
		/*
		 * Get a reference to the LN under the latch. Check the deleted flag in
		 * the BIN. If fetchTarget returns null, a deleted LN was cleaned.
		 */
		LN ln = null;
	...
	public OperationStatus getNext(DatabaseEntry foundKey,
			DatabaseEntry foundData, LockType lockType, boolean forward/*
																		 * Lck,
																		 * boolean
																		 * alreadyLatched
																		 */) throws DatabaseException {

		return getNextWithKeyChangeStatus(foundKey, foundData, lockType,
				forward/* Lck, alreadyLatched */).status;
	}
	 ...
	 * 
	 * @param foundKey
	 *            DatabaseEntry to use for returning key
	 * 
	 * @param foundData
	 *            DatabaseEntry to use for returning data
	 * 
	 * @param forward
	 *            if true, move forward, else move backwards
	 * 
	 * @param alreadyLatched
	 *            if true, the bin that we're on is already latched.
	 * 
	 * @return the status and an indication of whether we advanced to a new key
	 *         during the operation.
	 */
	public KeyChangeStatus getNextWithKeyChangeStatus(DatabaseEntry foundKey,
			DatabaseEntry foundData, LockType lockType, boolean forward
	/* Lck boolean alreadyLatched */) throws DatabaseException {

		assert assertCursorState(true) : dumpToString(true);
		// Lck assert checkAlreadyLatched(alreadyLatched) : dumpToString(true);

		KeyChangeStatus result = new KeyChangeStatus(OperationStatus.NOTFOUND,
				true);
		...
		try {
			while (bin != null) {
				...
				// Lck assert checkAlreadyLatched(alreadyLatched) :
				// dumpToString(true);
				// if (!alreadyLatched) {
				// Lck latchBIN();
				// } else {
				...
				if ((forward && ++index < bin.getNEntries())
					...
					OperationStatus ret = getCurrent(foundKey, foundData,
							lockType, forward);
					 ...
					 * traversal chain when we move onto the next BIN. When we
					 * traverse across BINs, there is a point when two BINs
					 * point to the same cursor.
					 * 
					 * Example: BINa(empty) BINb(empty) BINc(populated) Cursor
					 * (C) is traversing loop, leaving BINa: priorBIN is null, C
					 * points to BINa, BINa points to C set priorBin to BINa
					 * find BINb, make BINb point to C note that BINa and BINb
					 * point to C. loop, leaving BINb: priorBIN == BINa, remove
					 * C from BINa set priorBin to BINb find BINc, make BINc
					 * point to C note that BINb and BINc point to C finally,
					 * when leaving this method, remove C from BINb.
					 */
					if (binToBeRemoved != null) {
						hook_releaseBIN();
						flushBINToBeRemoved();
					...
					/*
					 * SR #12736 Prune away oldBin. Assert has intentional side
					 * effect
					 */
					assert TestHookExecute.doHookIfSet(testHook);

					if (forward) {
						newBin = database.getTree().getNextBin(binToBeRemoved,
								false /* traverseWithinDupTree */);
					} else {
						newBin = database.getTree().getPrevBin(binToBeRemoved,
								false /* traverseWithinDupTree */);
					}
					if (newBin == null) {
						...
						bin = newBin;
						// Lck alreadyLatched = true;
					}
				}
			}
		} finally {
			hook_assertNoLatches();
			if (binToBeRemoved != null) {
				flushBINToBeRemoved();
			}
		}
	...
	private void hook_verifyBin(BIN bin2) throws DatabaseException {
	...
	public OperationStatus getNextNoDup(DatabaseEntry foundKey,
		...
		// if (dupBin != null) {
		// Lck clearDupBIN(alreadyLatched);
		// alreadyLatched = false;
		// }

		return getNext(foundKey, foundData, lockType, forward/*
																 * Lck,
																 * alreadyLatched
																 */);
	}
	...
	/* Lck boolean alreadyLatched */) throws DatabaseException {
		...
		try {
			while (dupBin != null) {
				...
				hook_beginGetNextDuplicateLoop(dupBin);
				...
				if ((forward && ++dupIndex < dupBin.getNEntries())
					...
					if (foundKey != null) {
						ret = getCurrent(foundKey, foundData, lockType, forward);
					} else {
						hook_releaseDBIN();
					}
					if (ret == OperationStatus.SUCCESS) {
					...
					/*
					 * We need to go to the next DBIN. Remove the cursor and be
					 * sure to change the dupBin field after removing the
					 * cursor.
					 */
					if (dupBinToBeRemoved != null) {
						flushDBINToBeRemoved();
					...
					dupBin = null;
					// Lck dupBinToBeRemoved.releaseLatch();
					...
					// Lck dupBinToBeRemoved.latch();
					DBIN newDupBin;

					if (forward) {
						newDupBin = (DBIN) database
								.getTree()
								.getNextBin(dupBinToBeRemoved, true /* traverseWithinDupTree */);
					} else {
						newDupBin = (DBIN) database
								.getTree()
								.getPrevBin(dupBinToBeRemoved, true /* traverseWithinDupTree */);
					}
	...
	private void hook_beginGetNextDuplicateLoop(DBIN dupBin2) throws DatabaseException {
		// TODO Auto-generated method stub
		
	}

	private void hook_releaseDBIN() throws DatabaseException {
		// TODO Auto-generated method stub
		
	}

	private void hook_assertNoLatches() {
	}
				...
				if (in != null) {
					...
					if (bin.getNEntries() == 0) {
						...
						if (n != null && n.containsDuplicates()) {
							DIN dupRoot = (DIN) n;
							hook_latch(dupRoot);
							hook_releaseLatch(in);
							in = null;
							found = positionFirstOrLast(first, dupRoot);
						} else {

							hook_doesNotContainDuplicates(n);
							found = true;
						}
					}
				}
	...
	private void hook_doesNotContainDuplicates(Node n) {
		// TODO Auto-generated method stub
	...
	public int searchAndPosition(DatabaseEntry matchKey,
			DatabaseEntry matchData, SearchMode searchMode, LockType lockType)
			throws DatabaseException {
		...
		bin = (BIN) database.getTree().search(key, Tree.SearchType.NORMAL, -1,
				binBoundary, true /* updateGeneration */);

		if (bin != null) {
			addCursor(bin);

			/*
			 * If we're doing an exact search, tell bin.findEntry we require an
			 * exact match. If it's a range search, we don't need that exact
			 * match.
			 */
			index = bin.findEntry(key, true, exactSearch);

			/*
			 * If we're doing an exact search, as a starting point, we'll assume
			 * that we haven't found anything. If this is a range search, we'll
			 * assume the opposite, that we have found a record. That's because
			 * for a range search, the higher level will take care of sorting
			 * out whether anything is really there or not.
			 */
			...
			if (index >= 0) {
				if ((index & IN.EXACT_MATCH) != 0) {
					...
					/*
					 * The binary search told us we had an exact match. Note
					 * that this really only tells us that the key matched. The
					 * underlying LN may be deleted or the reference may be
					 * knownDeleted, or maybe there's a dup tree w/no entries,
					 * but the next layer up will find these cases.
					 */
					foundExactKey = true;
				...
				if (n != null) {
					...
					if (searchMode.isDataSearch()) {
						...
						if (!containsDuplicates && exactSearch) {
							...
							/*
							 * Note that we must not set the abort LSN for a
							 * read operation, lest false obsoletes are set.
							 * [13158]
							 */
						}
					}
				...
				/*
				 * Determine whether the last record was found. This is only
				 * possible when we don't lock the record, and when there are no
				 * duplicates.
				 */
				foundLast = (searchMode == SearchMode.SET_RANGE
						&& foundSomething && !containsDuplicates
		...
		return (foundSomething ? FOUND : 0) | (foundExactKey ? EXACT_KEY : 0)
	 ...
	 */
	private int searchAndPositionBoth(boolean containsDuplicates, Node n,
			DatabaseEntry matchData, boolean exactSearch, LockType lockType,
			long oldLsn) throws DatabaseException {
		...
		if (containsDuplicates) {
			...
			dupBin = (DBIN) database.getTree()
					.searchSubTree(duplicateRoot, data, Tree.SearchType.NORMAL,
							-1, null, true /* updateGeneration */);
			if (dupBin != null) {
				/* Find an exact match. */
				addCursor(dupBin);
				...
				if (dupIndex >= 0) {
					...
					/*
					 * The first duplicate is greater than the target data. Set
					 * index so that a 'next' operation moves to the first
					 * duplicate.
					 */
					dupIndex = -1;
					found = !exactSearch;
			...
			if (ln == null) {
				 ...
				 * for duplicates.
				 */
				int cmp = Key.compareKeys(ln.getData(), data, database
	...
	private OperationStatus fetchCurrent(DatabaseEntry foundKey,
		...
		if (targetIndex < 0 || targetIndex >= targetBin.getNEntries()
			...
			if (targetBin.isEntryPendingDeleted(targetIndex)) {
				...
				envImpl.addToCompressorQueue(targetBin, new Key(targetBin
						.getKey(targetIndex)), false);
			}
			...
			try {
				...
				hook_releaseLatchIfOwner(targetBin);
				throw DE;
			}
		}
		if (n == null) {
		...
		/*
		 * Note that since we have the BIN/DBIN latched, we can safely check the
		 * node type. Any conversions from an LN to a dup tree must have the bin
		 * latched.
		 */
		addCursor(targetBin);
		if (n.containsDuplicates()) {
			...
			DIN duplicateRoot = (DIN) n;
			hook_latch(duplicateRoot);
			hook_releaseLatch(targetBin);
		...
		/*
		 * Lock the LN. For dirty-read, the data of the LN can be set to null at
		 * any time. Cache the data in a local variable so its state does not
		 * change before calling setDbt further below.
		 */
		LockResult lockResult = lockLN(ln, lockType);
		return hook_fetchCurrentInternal(foundKey, foundData, lockType, first,
				lockResult, ln, duplicateFetch);
	}

	private void hook_fetchCurrentStart() throws DatabaseException  {
		
	}

	private OperationStatus hook_fetchCurrentInternal(DatabaseEntry foundKey,
			DatabaseEntry foundData, LockType lockType, boolean first,
			LockResult lockResult, LN ln,
			boolean duplicateFetch) throws DatabaseException {
		ln = lockResult.getLN();
		...
		/*
		 * Don't set the abort LSN here since we are not logging yet, even if
		 * this is a write lock. Tree.insert depends on the fact that the
		 * abortLSN is not already set for deleted items.
		 */
		if (duplicateFetch) {
			if (foundData != null) {
	...
	public LockResult lockLNDeletedAllowed(LN ln, LockType lockType)
		...
		/*
		 * Try a non-blocking lock first, to avoid unlatching. If the default is
		 * no-wait, use the standard lock method so LockNotHeldException is
		...
		if (locker.getDefaultNoWait()) {
			lockResult = locker.lock(ln.getNodeId(), lockType,
					true /* noWait */, database);
		} else {
			lockResult = locker.nonBlockingLock(ln.getNodeId(), lockType,
					database);
		}
		 ...
		 * the slot. If the node ID changed while unlatched, revert the lock and
		 * repeat.
		 */
		while (true) {
			...
			lockResult = locker.lock(nodeId, lockType, false /* noWait */,
					database);
	...
	private void hook_latchBINs() throws DatabaseException {
	}
	 ...
	 * 
	 * @param dupRoot
	 *            the duplicate root containing the DupCountLN to be locked.
	 * @param lockType
	 *            the type of lock requested.
	 ...
	 * 
	 * @param isDBINLatched
	 *            is true if the DBIN is currently latched.
	 */
	public DIN getDupRoot() throws DatabaseException {

		assert bin != null;
		assert index >= 0;

		return (DIN) bin.fetchTarget(index);
	}
	...
	public void lockEofNode(LockType lockType) throws DatabaseException {
		...
		locker.lock(database.getEofNodeId(), lockType, false /* noWait */,
				database);
	}
	...
	// */
	// private void trace(Level level, String changeType, BIN theBin, LN ln,
	// int lnIndex, long oldLsn, long newLsn) {
