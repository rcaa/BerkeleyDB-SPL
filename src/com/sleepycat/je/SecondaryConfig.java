 ...
 *
 * $Id: SecondaryConfig.java,v 1.1.6.1 2006/07/28 09:02:19 ckaestne Exp $
 */
...
public class SecondaryConfig extends DatabaseConfig {
	...
	void validate(DatabaseConfig configArg) throws DatabaseException {
		...
		boolean mkcMatch = equalOrBothNull(config.getMultiKeyCreator(),
				multiKeyCreator);
		...
		boolean fknMatch = equalOrBothNull(config.getForeignKeyNullifier(),
				foreignKeyNullifier);
		boolean fmknMatch = equalOrBothNull(config
				.getForeignMultiKeyNullifier(), foreignMultiKeyNullifier);
		boolean imskMatch = (config.getImmutableSecondaryKey() == immutableSecondaryKey);
		if (kcMatch && mkcMatch && fkdMatch && fkdaMatch && fknMatch
				&& fmknMatch && imskMatch) {
			return;
		}

		String message = genSecondaryConfigMismatchMessage(config, kcMatch,
				mkcMatch, fkdMatch, fkdaMatch, fknMatch, fmknMatch, imskMatch);
	...
	String genSecondaryConfigMismatchMessage(DatabaseConfig config,
			boolean kcMatch, boolean mkcMatch, boolean fkdMatch,
			boolean fkdaMatch, boolean fknMatch, boolean fmknMatch,
			boolean imskMatch) {
