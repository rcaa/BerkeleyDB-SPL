 ...
 *
 * $Id: DaemonThread.java,v 1.1.6.1.2.2 2006/08/29 10:17:53 ckaestne Exp $
 */
...
public abstract class DaemonThread implements DaemonRunner, Runnable {
	...
	/*
	 * Add an entry to the queue. Call this if the workQueueLatch is already
	 * held.
	 */
	...
	public void run() {
		while (true) {
			...
			try {
				boolean nothingToDo = getQueueSize() == 0;
				if (nothingToDo) {
					synchronized (synchronizer) {
						if (waitTime == 0) {
							synchronizer.wait();
