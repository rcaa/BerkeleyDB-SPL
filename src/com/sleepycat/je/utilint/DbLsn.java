 ...
 *
 * $Id: DbLsn.java,v 1.1.6.1 2006/07/28 09:02:50 ckaestne Exp $
 */
...
public class DbLsn {
	...
	public static String toString(long lsn) {
		return "<DbLsn val=\"0x" + Long.toHexString(getFileNumber(lsn)) + "/0x"
	 ...
	 */
	public static long getNoCleaningDistance(long thisLsn, long otherLsn,
			long logFileSize) {
		long diff = 0;
	...
	public static long getWithCleaningDistance(long thisLsn,
			FileManager fileManager, long otherLsn, long logFileSize) {
		long diff = 0;
		...
		if (myFile == otherFile) {
			...
			if (myFileIdx > otherFileIdx) {
				diff = calcDiff(myFileIdx - otherFileIdx, logFileSize, thisLsn,
						otherLsn);
			} else {
				diff = calcDiff(otherFileIdx - myFileIdx, logFileSize,
						otherLsn, thisLsn);
			}
		}
		return diff;
	...
	private static long calcDiff(long fileDistance, long logFileSize,
			long laterLsn, long earlierLsn) {
