 ...
 *
 * $Id: INCompressor.java,v 1.1.2.4 2007/03/01 23:15:23 ckaestne Exp $
 */
...
public class INCompressor extends DaemonThread {
	...
	/*
	 * The following stats are not kept per run, because they're set by multiple
	 * threads doing lazy compression. They are debugging aids; it didn't seem
	 * like a good idea to add synchronization to the general path.
	 */
	...
	/*
	 * There are multiple flavors of the addBin*ToQueue methods. All allow the
	 * caller to specify whether the daemon should be notified. Currently no
	 * callers proactively notify, and we rely on lazy compression and the
	 * daemon timebased wakeup to process the queue.
	 */
	...
	/**
	 * Adds an entire collection of BINReferences to the queue at once. Use this
	 * to avoid latching for each add.
	 */
	public void addMultipleBinRefsToQueue(Collection binRefs, boolean doWakeup)
			throws DatabaseException {
		...
		synchronized (binRefQueueSync) {
			...
			if (target != null) {
				if (target.deletedKeysExist()) {
					...
					/*
					 * This is an entry that needs to be pruned. Put it back to
					 * be dealt with by the daemon.
					 */
	...
	protected int nDeadlockRetries() throws DatabaseException {
		...
		return env.getConfigManager()
				.getInt(EnvironmentParams.COMPRESSOR_RETRY);
	}
	...
	public synchronized void doCompress() throws DatabaseException {
		...
		if (binQueueSize > 0) {
			hook_internalDoCompress(queueSnapshot);
		}
	}

	private void hook_internalDoCompress(Map queueSnapshot)
			throws DatabaseException {

		// refined trace Tracer.trace(Level.FINE, env,
		// "InCompress.doCompress called, queue size: " + binQueueSize);
		// Lck assert LatchSupport.countLatchesHeld() == 0;

		/*
		 * Compressed entries must be counted as obsoleted. A separate tracker
		 * is used to accumulate tracked obsolete info so it can be added in a
		 * single call under the log write latch. We log the info for deleted
		 * subtrees immediately because we don't process deleted IN entries
		 * during recovery; this reduces the chance of lost info.
		 */
		...
		while (it.hasNext()) {
			...
			if (!findDBAndBIN(binSearch, binRef, dbTree, dbCache)) {
				...
				/*
				 * Either the db is closed, or the BIN doesn't exist. Don't
				 * process this BINReference.
				 */
				continue;
			}
			...
			if (binRef.deletedKeysExist()) {
				...
				boolean requeued = compressBin(binSearch.db, binSearch.bin,
						binRef, tracker);

				if (!requeued) {

					/*
					 * This BINReference was fully processed, but there may
					 * still be deleted slots. If there are still deleted keys
					 * in the binref, they were relocated by a split.
					 */
				...
				/*
				 * An empty BINReference on the queue was put there by a lazy
				 * compressor to indicate that we should try to prune an empty
				 * BIN.
				 */
				...
				/*
				 * Release the bin latch taken by the initial search. Pruning
				 * starts from the top of the tree and requires that no latches
				 * are held.
				 */
				// Lck foundBin.releaseLatch();
		 ...
		 * recovery. All latches must have been released.
		 */
	 ...
	 * 
	 * @param bin
	 *            is latched when this method is called, and unlatched when it
	 *            returns.
	...
	private boolean pruneBIN(DatabaseImpl dbImpl, BINReference binRef,
		...
		try {
			...
			/*
			 * Something was added to the node since the point when the deletion
			 * occurred; we can't prune, and we can throw away this
			 * BINReference.
			 */
		} catch (CursorsExistException e) {

			/*
			 * If there are cursors in the way of the delete, retry later. For
			 * example, When we delete a BIN or DBIN, we're guaranteed that
	...
	private void checkForRelocatedSlots(DatabaseImpl db, BINReference binRef,
		...
		if (iter != null) {
			...
			while (iter.hasNext()) {
				 ...
				 * separately.
				 */
				BIN splitBin = isDup ? searchForBIN(db, mainKey, key.getKey())
	 ...
	 * @return the BIN or DBIN that corresponds to this BINReference. The node
	 *         is latched upon return. Returns null if the BIN can't be found.
	 */
	...
	private BIN searchForBIN(DatabaseImpl db, byte[] mainKey, byte[] dupKey)
		...
		IN in = tree
				.search(mainKey, SearchType.NORMAL, -1, null, false /* updateGeneration */);
		...
		try {
			...
			if (index >= 0) {
				...
				if (node.containsDuplicates()) {
					...
					// Lck bin.releaseLatch();
					duplicateBin = (DBIN) tree
							.searchSubTree(duplicateRoot, dupKey,
									SearchType.NORMAL, -1, null, false /* updateGeneration */);

					return duplicateBin;
				} else {
					/*
					 * We haven't migrated to a duplicate tree yet. XXX, isn't
					 * this taken care of above?
					 */
					return bin;
				}
			} else {
			...
			// if (bin != null) {
			// Lck bin.releaseLatchIfOwner();
			// }
			// if (duplicateRoot != null) {
			// Lck duplicateRoot.releaseLatchIfOwner();
			// }

			/*
			 * FindBugs whines about Redundent comparison to null below, but for
			 * stylistic purposes we'll leave it in.
			 */
			// if (duplicateBin != null) {
			// Lck duplicateBin.releaseLatchIfOwner();
			// }
			throw DBE;
		}
	}
	...
	/**
	 * Lazily compress a single BIN. Do not do any pruning. The target IN should
	 * be latched when we enter, and it will be remain latched.
	 */
	public void lazyCompress(IN in) throws DatabaseException {
		...
		if (nCursors > 0) {
			...
			BINReference binRef = removeCompressibleBinReference(bin
					.getNodeId());
			if ((binRef == null) || (!binRef.deletedKeysExist())) {
				 ...
				 * remaining, requeue, so the daemon can handle this. Either we
				 * must have shuffled some items because of a split, or a child
				 * was not resident and we couldn't process that entry.
				 */
	...
	/*
	 * Find the db and bin for a BINReference. @return true if the db is open
	 * and the target bin is found.
	 */
	private boolean findDBAndBIN(BINSearch binSearch, BINReference binRef,
			DbTree dbTree, Map dbCache) throws DatabaseException {

		/* Find the database. */
		binSearch.db = dbTree.getDbCaching(binRef.getDatabaseId(), lockTimeout,
				dbCache);
		/* The db was deleted. Ignore this BIN Ref. */
		if (!hook_getDbExists(binSearch.db)) {
			return false;
		}
		...
		if ((binSearch.bin == null)
			...
			if (binSearch.bin != null) {
				// Lck binSearch.bin.releaseLatch();
			}
			return false;
		}
	...
	private boolean hook_getDbExists(DatabaseImpl db) {
		return db != null;
	}
